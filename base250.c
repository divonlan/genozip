...
Base250 base250_encode (uint32_t n, // number to encode
                        Base250Encoding minimum_encoding) // optimization - we skip encoding for lower bits if we already know we won't be using them
{
    ...
    if (minimum_encoding <= B250_ENC_8) {
        if (num_numerals > 1) {
    ...
    // calculate the 16bit encoding
    if (minimum_encoding <= B250_ENC_16) {
        switch (num_numerals) {
            case 1:
            ...
            case 4:
                ...
                result.numerals[B250_ENC_16][0]  = (num_numerals == 3) ? BASE250_3_NUMERALS : BASE250_4_NUMERALS;
    ...
    // calculate the 24bit encoding (always, regardless of minimum_encoding)
    switch (num_numerals) {
        case 1:
            if (result.n==0) { // #1 most frequent snip
...
void base250_unit_test()
{
    ...
    for (unsigned i=0; i<4; i++) {
        ...
        Base250 b = base250_encode (n[i], B250_ENC_8);
