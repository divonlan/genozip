...
static inline char *_word_to_str (uint64_t word, char str[WORD_SIZE+1])
...
static inline char *_word_to_str (uint64_t word, char str[WORD_SIZE+1])
{
    ...
    for (i = 0; i < WORD_SIZE; i++)
        str[WORD_SIZE-i-1] = ((word >> i) & (uint64_t)0x1) == 0 ? '0' : '1';
...
#define DEBUG_VALIDATE(a) validate_bitarr((a), __FUNCLINE)
static inline void validate_bitarr (const BitArray *arr, rom file, int lineno)
{
...
static inline void _set_word (BitArray *bitarr, uint64_t start, uint64_t word)
{
    ...
    // bit_array_clear_excess_bits_in_top_word(bitarr); // divon: removed bc not thread safe when setting different parts of the bit array with multiple threads
}

static inline void _set_byte (BitArray *bitarr, uint64_t start, uint8_t byte)
{
    uint64_t w = _get_word(bitarr, start);
    _set_word(bitarr, start, (w & ~(uint64_t)0xff) | byte);
}

// Wrap around
static inline uint64_t _get_word_cyclic (const BitArray *bitarr, uint64_t start)
{
    uint64_t word = _get_word(bitarr, start);

    uint64_t bits_taken = bitarr->nbits - start;

    if (bits_taken < WORD_SIZE) {
        word |= (bitarr->words[0] << bits_taken);

        if (bitarr->nbits < (uint64_t)WORD_SIZE)
...
// Wrap around
static inline void _set_word_cyclic (BitArray *bitarr, uint64_t start, uint64_t word)
{
    _set_word(bitarr, start, word);

    uint64_t bits_set = bitarr->nbits - start;
...
static inline void _set_region (BitArray *bitarr, uint64_t start, uint64_t length, FillAction action)
{
    if (length == 0) return;
...
BitArray bit_array_alloc_do (uint64_t nbits, bool clear, FUNCLINE)
{
    BitArray bitarr = { .type   = BITARR_STANDALONE, // standalone bitarray that is not part of a Buffer
                        .nbits  = nbits,
                           ...
                           bool clear, FUNCLINE)
{
    ...
    bitarr->nbits  = nbits;
    bitarr->nwords = roundup_bits2words64 (nbits); // possibly less than low_level_nbits
    /*xxx

    *bitarr = (BitArray){
      // note: over-allocation is at the libc level, with no difference in the BitArray structure. next allocation will be faster.
      .words  = (bitarr->type == BITARR_UNALLOCATED) ? buf_low_level_malloc (size, false, func, code_line)
                                                     : buf_low_level_realloc (bitarr->words, size, "", func, code_line),
      .nbits  = nbits,
      .nwords = roundup_bits2words64 (nbits),
      .type   = BITARR_REGULAR
    };
*/
    if (clear && nbits >old_nbits) 
        bit_array_clear_region (bitarr, old_nbits, nbits - old_nbits);
...
uint64_t bit_array_length (const BitArray *bit_arr)
{
    return bit_arr->nbits;
}

// If bitarr length < num_bits, resizes to num_bits
char bit_array_ensure_size(BitArray *bitarr, uint64_t ensure_num_of_bits)
{
...
static inline void bit_array_ensure_size_critical(BitArray *bitarr, uint64_t nbits)
{
...
char bit_array_get_bit(const BitArray *bitarr, uint64_t b)
{
...
void bit_array_set_bit(BitArray *bitarr, uint64_t b)
{
...
void bit_array_clear_bit(BitArray *bitarr, uint64_t b)
{
    assert(b < bitarr->nbits);
...
void bit_array_toggle_bit(BitArray *bitarr, uint64_t b)
{
    assert(b < bitarr->nbits);
...
void bit_array_assign_bit(BitArray *bitarr, uint64_t b, char c)
{
    assert(b < bitarr->nbits);
...
uint64_t bit_array_get_bits(const BitArray *bitarr,
                               ...
                               uint64_t *dst)
{
    ...
    for (i = start; i < end; i++) 
        if (bit_array_get(bitarr, i)) dst[n++] = i;
...
void bit_array_set_region (BitArray *bitarr, uint64_t start, uint64_t len)
{
...
void bit_array_clear_region_do (BitArray *bitarr, uint64_t start, uint64_t len, rom func, unsigned code_line)
{
    if (!len) return; // nothing to do 
...
uint64_t bit_array_get_wordn(const BitArray *bitarr, uint64_t start, int n /* up to 64 */)
{
...
void bit_array_set_word64 (BitArray *bitarr, uint64_t start, uint64_t word)
{
    ASSERT (start < bitarr->nbits, "expecting start(%"PRIu64") < bitarr->nbits(%"PRIu64")", start, bitarr->nbits);
    _set_word(bitarr, start, (uint64_t)word);
}

void bit_array_set_word32 (BitArray *bitarr, uint64_t start, uint32_t word)
{
    ASSERT (start < bitarr->nbits, "expecting start(%"PRIu64") < bitarr->nbits(%"PRIu64")", start, bitarr->nbits);
    uint64_t w = _get_word(bitarr, start);
...
void bit_array_set_word16 (BitArray *bitarr, uint64_t start, uint16_t word)
{
    ASSERT (start < bitarr->nbits, "expecting start(%"PRIu64") < bitarr->nbits(%"PRIu64")", start, bitarr->nbits);
    uint64_t w = _get_word(bitarr, start);
...
void bit_array_set_word8 (BitArray *bitarr, uint64_t start, uint8_t byte)
{
    ASSERT (start < bitarr->nbits, "expecting start(%"PRIu64") < bitarr->nbits(%"PRIu64")", start, bitarr->nbits);
    _set_byte(bitarr, start, byte);
}

void bit_array_set_wordn(BitArray *bitarr, uint64_t start, uint64_t word, int n)
{
    ASSERT (start < bitarr->nbits, "expecting start(%"PRIu64") < bitarr->nbits(%"PRIu64")", start, bitarr->nbits);
    assert(n <= 64);
    uint64_t w = _get_word(bitarr, start), m = bitmask64(n);
...
bool bit_array_is_fully_set (const BitArray *bitarr)
{
    if (!bitarr->nbits) return true; // trivially true

    // all words but last one
    for (uint64_t i=0; i < bitarr->nwords-1; i++)
        if (bitarr->words[i] != 0xffffffffffffffffULL) return false;

    // last word might be partial
    word_offset_t bits_active = bits_in_top_word (bitarr->nbits);    
    word_offset_t num_of_bits_set = POPCOUNT (bitarr->words[bitarr->nwords-1] & bitmask64 (bits_active));

    return num_of_bits_set == bits_active;
}

// Get the number of bits set (hamming weight)
uint64_t bit_array_num_bits_set (const BitArray *bitarr)
{
...
uint64_t bit_array_num_bits_cleared (const BitArray *bitarr)
{
...
bool FUNC(const BitArray *bitarr, uint64_t offset, uint64_t *result) \
{ \
    ...
    uint64_t w = GET(bitarr->words[i]) & ~bitmask64(bitset64_idx(offset)); \
    \
    while(1) { \
        if (w > 0) { \
...
{ \
    ...
    uint64_t w = GET(bitarr->words[i]) & bitmask64(bitset64_idx(offset-1)+1); \
    \
...
bool bit_array_find_first_set_bit (const BitArray *bitarr, uint64_t *result)
{
    return bit_array_find_next_set_bit(bitarr, 0, result);
}

// same same
bool bit_array_find_first_clear_bit (const BitArray *bitarr, uint64_t *result)
{
...
// If no bits are set, value at `result` is not changed
bool bit_array_find_last_set_bit (const BitArray *bitarr, uint64_t *result)
{
    return bit_array_find_prev_set_bit(bitarr, bitarr->nbits, result);
}

// same same
bool bit_array_find_last_clear_bit (const BitArray *bitarr, uint64_t *result)
{
...
void bit_array_from_substr(BitArray *bitarr, uint64_t offset,
                           rom str, size_t len,
                           rom on, rom off,
                           char left_to_right)
{
...
// From string method
void bit_array_from_str (BitArray *bitarr, rom str)
{
...
{
    for (uint64_t i = 0; i < bitarr->nbits; i++)
...
void bit_array_print_do (const BitArray *bitarr, rom msg, FILE *file)
{
...
void bit_array_print_binary_word_do (uint64_t word, rom msg, FILE *file)
{
...
void bit_array_print_substr (rom msg, 
                             ...
                             FILE *file)
{
    ...
    for (uint64_t i=0; i < length; i++) 
        fputc (bit_array_get(bitarr, start + i) ? '1' : '0', file);

    fputc ('\n', file);
}

// prints a 2bit bases array - A,C,G,T bases
void bit_array_print_substr_bases (rom msg, 
                                   const BitArray *bitarr,
                                   uint64_t start, uint64_t length, // in bases, not bits
                                   FILE *file)
{
    start *= 2; // now it is in bits
    length = MIN_(length * 2, bitarr->nbits - start);

    if (msg) fprintf (file, "%s: ", msg);
    for (uint64_t i=0; i < length; i += 2) { 
        char b = bit_array_get2 (bitarr, start + i);
...
static void _array_copy (BitArray *dst, uint64_t dstindx,
                         const BitArray *src, uint64_t srcindx,
                         uint64_t length)
{
...
// destination and source may be the same bit_array
// and src/dst regions may overlap
void bit_array_copy_do (BitArray *dst, uint64_t dstindx,
                        const BitArray *src, uint64_t srcindx,
                        uint64_t length, FUNCLINE)
{
    DEBUG_VALIDATE(src);
    ...
    if (dstindx + length == dst->nbits)
        bit_array_clear_excess_bits_in_top_word(dst);

    DEBUG_VALIDATE(dst);
}

// concatenate src at the end of dst (divon)
void bit_array_concat_do (BitArray *base, const BitArray *add, 
                          unsigned additional_concats_expected, FUNCLINE) // only used for non-buffer: low-level realloc more than needed, expecting additional concats - improves perforamce
{
    uint64_t index = base->nbits;

    // IMPORTANT: if calling with a bitarray that is embedded in a buffer, buf_add_to_buffer_list must be 
    // called first (eg in seg_initialize), as bit_array_realloc_do performs an anonymous realloc 
    bit_array_realloc_do (base, 
...
/*    BufferP buf = buf_get_buffer_from_bit_array (base);
    ...
    bit_array_clear_excess_bits_in_top_word (base);
*/
    bit_array_copy (base, index, add, 0, add->nbits);
}

void bit_array_overlay (BitArray *overlaid_bitarr, BitArray *regular_bitarr, uint64_t start, uint64_t nbits)
{
...
/*
void bit_array_or_with (BitArray *dst, uint64_t dst_start_bit, BitArray *src , uint64_t src_start_bit, uint64_t len)
{
    uint64_t dst_word, src_word;
    uint64_t i, last_word_bits = len % WORD_SIZE; 
...
{
    uint64_t min_words = MAX_(bitarr1->nwords, bitarr2->nwords);
...
int bit_array_cmp_words(const BitArray *arr1,
                        uint64_t pos, const BitArray *arr2)
{
    if (arr1->nbits == 0 && arr2->nbits == 0) return 0;

    uint64_t top_bit1 = 0, top_bit2 = 0;

    bool arr1_zero = !bit_array_find_last_set_bit(arr1, &top_bit1);
    bool arr2_zero = !bit_array_find_last_set_bit(arr2, &top_bit2);
...
static void _reverse_region (BitArray *bitarr, uint64_t start, uint64_t length)
{
    uint64_t left = start;
    uint64_t right = (start + length - WORD_SIZE) % bitarr->nbits;

    while(length >= 2 * WORD_SIZE) {
        // Swap entire words
        uint64_t left_word  = _get_word_cyclic(bitarr, left);
        uint64_t right_word = _get_word_cyclic(bitarr, right);
...
void bit_array_reverse_region(BitArray *bitarr, uint64_t start, uint64_t len)
{
...
{
    if (bitarr->nbits > 0) {
        _reverse_region (bitarr, 0, bitarr->nbits);
        bit_array_clear_excess_bits_in_top_word (bitarr); // divon: work-around - sometimes _reverse_region contaminates the top word, not clear why. TO DO: fix it.
    }
...
void bit_array_reverse_complement_aligned (BitArray *dst, const BitArray *src,
                                           uint64_t src_start_base, uint64_t max_num_bases) 
{
...
void bit_array_reverse_complement_in_place (BitArray *bitarr) 
{
    DEBUG_VALIDATE(bitarr);

    bit_array_reverse (bitarr); // after the reverse - A=00 C=10 G=01 T=11 - so effectively C and G are already complemented, and we need to just do A and T

    uint8_t *bytes = (uint8_t *)bitarr->words;
    uint64_t nbytes = bitarr->nwords * sizeof(uint64_t);
...
void bit_array_shift_left(BitArray *bitarr, uint64_t shift_dist, char fill)
{
...
// Shift towards LSB / lower index
void bit_array_shift_right_shrink (BitArray *bitarr, uint64_t shift_dist) // added by divon
{
    if (shift_dist >= bitarr->nbits) {
...
void bit_array_remove_flanking (BitArray *bitarr, uint64_t lsb_flanking, uint64_t msb_flanking) // added by divon
{
    DEBUG_VALIDATE (bitarr); // catch a bug

    uint64_t cpy_length = bitarr->nbits - lsb_flanking;
...
void bit_array_truncate (BitArray *bitarr, uint64_t new_num_of_bits)
{
...
uint64_t bit_array_effective_length (BitArray *bitarr)
{
    int64_t i;
    ...
    uint64_t last_non_zero_word = bitarr->words[i];
...
static inline uint64_t _bit_array_combined_word (uint64_t word_a, uint64_t word_b, uint8_t shift)
{
...
uint32_t bit_array_hamming_distance (const BitArray *bitarr_1, uint64_t index_1, 
                                     const BitArray *bitarr_2, uint64_t index_2, 
                                     uint64_t len)
{
    const uint64_t *words_1 = &bitarr_1->words[index_1 >> 6];
    ...
    const uint64_t *words_2 = &bitarr_2->words[index_2 >> 6];
    ...
    for (uint32_t i=0; i < nwords; i++) {
        ...
        uint64_t word_1 = shift_1 ? _bit_array_combined_word (words_1[i], words_1[i+1], shift_1) : words_1[i];
        uint64_t word_2 = shift_2 ? _bit_array_combined_word (words_2[i], words_2[i+1], shift_2) : words_2[i];
...
void bit_array_cycle_right (BitArray *bitarr, uint64_t cycle_dist)
{
    if (bitarr->nbits == 0) return;

    cycle_dist = cycle_dist % bitarr->nbits;

    if (cycle_dist == 0) return;

    uint64_t len1 = cycle_dist;
    uint64_t len2 = bitarr->nbits - cycle_dist;
...
void bit_array_cycle_left (BitArray *bitarr, uint64_t cycle_dist)
{
    if (bitarr->nbits == 0) return;
    ...
    uint64_t len1 = bitarr->nbits - cycle_dist;
    uint64_t len2 = cycle_dist;
