...
static bool sam_seg_prim_add_sa_group (VBlockSAMP vb, ZipDataLineSAM *dl, uint16_t num_alns/* inc primary aln*/, bool is_bam)
{
    ...
    if (sam_is_main_vb) {
        ...
        FAILIF (!str_is_only_ACGT (textual_seq, seq_len, &bad_i), 
                "RNAME=\"%.*s\" POS=%"PRIu64". Found '%c' in base_i=%u in SEQ is not A,C,G or T. SEQ=\"%.*s\"", 
...
// Call in seg PRIM line (both in MAIN and PRIM vb): 
// MAIN: test that line is a valid prim before moving it to the PRIM gencomp file
// PRIM: add SA Group (including alignments) to the data structure in VB (called from sam_seg_SA_field)
bool sam_seg_prim_add_sa_group_SA (VBlockSAMP vb, ZipDataLineSAM *dl, STRp(sa), int64_t this_nm, bool is_bam)
{
    ...
    FAILIF (dl->POS  > MAX_SA_POS, "POS=%"PRIu64" > MAX_SA_POS=%u", dl->POS, MAX_SA_POS);
...
bool sam_seg_is_gc_line (VBlockSAMP vb, ZipDataLineSAM *dl, STRp(alignment), STRps(aux), bool is_bam)
{
    ...
    else if ((SA = sam_seg_get_aux_str (vb, "SA:Z", STRas(aux), &SA_len, is_bam)) &&
             sam_seg_get_aux_int (vb, "NM:i", STRas(aux), &NM, is_bam) &&
             sam_seg_prim_add_sa_group_SA (vb, dl, STRa (SA), NM, is_bam))  // testing to see if we can successfully add an SA Group based on SA
    ...
    else if (sam_seg_get_aux_int (vb, "NH:i", STRas(aux), &NH, is_bam) && NH > 1 &&
             sam_seg_get_aux_int (vb, "HI:i", STRas(aux), &HI, is_bam) && HI == 1 &&
                                            ...
                                            bool is_bam)
{
    ...
    BitArray *prim_seq = buf_get_bitarray (&z_file->sa_seq); // ACTG format
    ...
    if (!xstrand)
        for (uint32_t i=0; i < depn_seq_len; i++) {
            ...
            switch (depn_textual_seq[i]) {
                ...
                default : return false; // depn seq has a non-ACTG "base" - we can't seg it against prim
            }
        }
...
static inline bool sam_sa_seg_depn_get_my_alns (VBlockSAMP vb,
                                                WordIndex my_rname_contig, PosType my_pos, uint8_t my_mapq, 
...
{
    #define DONE ({ if (vb->hard_clip[0] || vb->hard_clip[1])           \
                        sam_cigar_S_to_H ((char*)STRa(textual_cigar));  \
                    return; })
    ...
    if (vb->hard_clip[0] || vb->hard_clip[1])
        sam_cigar_H_to_S ((char*)STRa(textual_cigar));
        ...
        if (dl->FLAG.bits.is_first == g->is_first && // note: order tests in the condition is optimized to fail fast with minimal effort
            ...
            if (flag.show_depn)
                 iprintf ("vb=%u grp=%u aln=%s\n", vb->vblock_i, ZGRP_I(g), 
                          SA ? sam_show_sa_one_aln (vb->sa_grp, vb->sa_aln).s : "N/A");                
                             ...
                             STRps(aux), STRp(textual_cigar), rom textual_seq, bool is_bam)
{
    ...
    if (sam_is_depn_vb) 
        ...
        vb->NM = 0; // default
        uint32_t NM_len = sam_seg_get_aux_int (vb, "NM:i", STRas(aux), &vb->NM, is_bam);

        if (sam_seg_get_aux_str (vb, "SA:Z", STRas(aux), NULL, is_bam)) {
            ctx_set_encountered (VB, CTX(OPTION_SA_Z)); // for DEPN, this is done in sam_sa_seg_depn_find_sagroup

            // NM is needed by sam_seg_SA_field
            vb->NM_len = !is_bam/*SAM: \t or \n*/ + NM_len;
        }
...
{
    SNIP(24) = { SNIP_SPECIAL, SAM_SPECIAL_SAGROUP };
    snip_len = 2 + str_int (parameter, &snip[2]);
