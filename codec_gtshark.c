...
static void *codec_gtshark_read_gtshark_output_file (void *arg)
{
    ...
    uint32_t bytes_read;
    do {
        buf_alloc_old (vb, buf, buf->len + CHUNK, 2, "contexts->local"); 
...
void codec_gtshark_comp_init (VBlock *vb_)
{
    ...
    vb->gtshark_gt_ctx->flags.ctxs_dot_is_0 = vb->ht_matrix_ctx->flags.ctxs_dot_is_0;
...
static void *codec_gtshark_zip_create_vcf_file (void *arg)
{
    ...
    buf_alloc_old (vb, &vb->gtshark_ex_ctx->local, vb->lines.len * num_hts, 1.1, "contexts->local");
    ...
    buf_alloc_old (vb, &vb->compressed, vardata_len + num_hts*2, 1.2, "compressed");
                             ...
                             bool soft_fail)           // soft fail not supported
{
    ...
    int vcf_thread, db_thread, gt_thread;
    ...
    vcf_thread = threads_create (codec_gtshark_zip_create_vcf_file, &vcf_thread_arg, "gtshark_vcf_threads", vb->vblock_i);
    ...
    db_thread = threads_create (codec_gtshark_read_gtshark_output_file, &db_read_thread_arg, "gtshark_db_threads", vb->vblock_i);

    RWThreadArg gt_read_thread_arg = { vb, gtshark_gt_name, &vb->gtshark_gt_ctx->local };
    gt_thread = threads_create (codec_gtshark_read_gtshark_output_file, &gt_read_thread_arg, "gtshark_gt_threads", vb->vblock_i);
...
// backward compatability: in versions 10.0.3-10.0.8 '.' in the GT field (". \b" in the ht_matrix) was written
// with "00" if the file has no "00". this was removed as it didn't occur in any of our files (it should have
// been replacing ./. and not . - but even then it didn't help in real world files since almost all VBs had some 00)
static void codec_gtshark_recover_dots_from_00 (VBlockVCFP vb, ContextP ctx)
{
    ARRAY (uint8_t, ht_matrix, vb->ht_matrix_ctx->local);

    // recover '..' from '00' (or '.' from '0') if needed
    if (ctx->flags.ctxs_dot_is_0) {        
        if (vb->ploidy == 1)
            ...
            ABORT ("Error in codec_gtshark_recover_dots_from_00: ctxs_dot_is_0 not expected for ploidy=%u larger than 2", vb->ploidy);
    }
}
...
static void codec_gtshark_reconstruct_ht_matrix (VBlockVCF *vb)
{
    ...
    // recover '..' from '00' (or '.' from '0') if needed
    codec_gtshark_recover_dots_from_00 (vb, vb->gtshark_gt_ctx);
...
void codec_gtshark_uncompress (VBlock *vb_, Codec codec, uint8_t param,
                               ...
                               Codec sub_codec)
{
    ...
    vb->ht_matrix_ctx->ltype  = LT_CODEC; // reconstruction will go to codec_pbwt_reconstruct as defined in codec_args for CODEC_GTSHARK
    ...
    db_thread = threads_create (codec_gtshark_write_gtshark_input_file, &db_write_thread_arg, "gtshark_db_thread", vb->vblock_i);

    RWThreadArg gt_write_thread_arg = { vb, gtshark_gt_name, &vb->gtshark_gt_ctx->local };
    gt_thread = threads_create (codec_gtshark_write_gtshark_input_file, &gt_write_thread_arg, "gtshark_gt_thread", vb->vblock_i);
    ...
    vcf_thread = threads_create (codec_gtshark_read_gtshark_output_file, &vcf_read_thread_arg, "gtshark_vcf_thread", vb->vblock_i);
