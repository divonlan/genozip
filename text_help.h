...
static const char *help_genounzip[] = {
    ...
    "   -c --stdout       Send output to standard output instead of a file",
    "",
    "   -z --bgzip        Compress the output to bgzip format (.gz extension).",
    "                     Note: if the original file was bgzip-compressed (as .bam, .fastq.gz and .vcf.gz usually) are we attempt to re-compress to the same byte-level identical compressed format as the source, using libdeflate level 6. However, the bgzip-compression might be slighly different if the source file was compressed with a different compression library or options.",
    "                     Note: this option is implicit if --output specifies a filename ending with .vcf.gz or .vcf.bgz",
    "                     Note: this option is implicit if the original file was compressed with bgzip, unless output is to stdout (i.e. the terminal or a pipe)",
    "",
    "      --plain        Outputs a plain (not BGZIPped) file - negates implificit --bgzip",
    "",
