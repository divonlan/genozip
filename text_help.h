...
static const char *help_genozip[] = {
    ...
    "",
    "Optimizing:",    
    "   -9 --optimize     Modify the file in ways that are likely insignificant for analytical purposes, but make a significant difference for compression. At the moment, these optimizations are:",    
    "",
    "                     For VCF files: ",
...
static const char *help_genozip_developer[] = {
    ...
    "",
    "   Z    --show-alleles    Output allele values to stdout. Each row corresponds to a row in the VCF file. Mixed-ploidy regions are padded, and 2-digit allele values are replaced by an ascii character",
    "",
    "   ZU   --show-dict       Show dictionary fragments written for each vblock (works for genounzip too)",
    "",
    "   ZU   --show-one-dict   <field-name>. Show the dictionary for this field in a tab-separated list - <field-name> may be one of the fields 1-9 (CHROM to FORMAT) or a INFO tag or a FORMAT tag (works for genounzip too)",
    "",
    "   Z    --show-gt-nodes   Show transposed GT matrix - each value is an index into its dictionary",
    "",
    ...
    "",
    "   ZUC  --show-index      Show the content of the random access index. When used with genocat, only the index is shown, not the data",
    "",
    "   Z    --show-gheader    Show the content of the genozip header (which also includes the list of all sections in the file)",
    "",
...
static const char *help_genocat[] = {
    "",
    "Print original file(s) (VCF or SAM) previously compressed with genozip",
    "",
    ...
    "",
    "Options:",    
    "VCF SAM -r --regions      [^]chr|chr:pos|pos|chr:from-to|chr:from-|chr:-to|from-to|from-|-to[,...]",
    ...
    "",
    "VCF SAM -t --targets           Identical to --regions, provided for pipeline compatibility",
    "",
    "VCF     -s --samples      [^]sample[,...]",
    ...
    "",
    "VCF     -G --drop-genotypes Output the data without the individual genotypes and FORMAT column",
    "",
    "VCF SAM -H --no-header    Don't output the header",
    "",
    "VCF     -1 --header-one   Don't output the VCF header, except for the last line (with the field and sample names)",
    "",
    "VCF SAM    --header-only  Output only the header",
    "",
    "VCF        --GT-only      For samples, output only genotype (GT) data, dropping the other subfields",
    "",
    "VCF SAM    --strip        Don't output values for ID, QUAL, FILTER, INFO; FORMAT is only GT (at most); Samples include allele values (i.e. GT subfield) only",
    "",
