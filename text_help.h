...
static const char *help_genozip[] = {
    "",
    "Compress genomics files. Currently supported file types: VCF/BCF and SAM/BAM",
    "",
    ...
    "",    
    "Flags:",    
    "   -i --input        <data-type>. data-type is one of: ",
    "                        VCF: vcf, vcf.gz, vcf.bgz, vcf.bz2, vcf.xz, bcf, bcf.gz, bcf.bgz",
    "                        SAM: sam, bam",
...
static const char *help_genozip_developer[] = {
    ...
    "",
    "   ZUC  --show-dict       Show dictionary fragments written for each vblock (works for genounzip too). When used with genocat, only the index is shown, not the data",
    "",
    "   ZUC  --show-one-dict   <field-name>. Show the dictionary for this field in a tab-separated list - <field-name> may be one of the fields 1-9 (CHROM to FORMAT) or a INFO tag or a FORMAT tag (works for genounzip too). When used with genocat, only the index is shown, not the data",
    "",
...
static const char *help_genocat[] = {
    ...
    "",
    "VCF SAM    --strip        Display only genotype (VCF) / sequence (SAM) and minimal additional information",
    "",
