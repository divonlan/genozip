...
static const char *help_genozip[] = {
    ...
    "",
    "    --sort           (VCF only) Causes genozip to generate a \"reconstruction plan\" that will allow genocat to show the file sorted. This is designed for mildly-unsorted files. If the file is highly unsorted, this might result in genocat loading a big portion of the uncompressed file to memory (genocat --unsorted can be used to prevent sorting). This option is always set for dual-coordinates files unless overridden with --unsorted",
    "",
...
static const char *help_genounzip[] = {
    ...
    "   -^ --replace      Replace the source file with the result file, rather than leaving it unchanged",    
    "",
    "   -u --unbind=prefix Specify a prefix that is added to each file component name. A prefix may include a directory",
    "",
