...
RefLock ref_lock (PosType gpos_start, uint32_t seq_len)
{
    ...
    ASSERT (lock.first_mutex >= 0 && lock.first_mutex <= genome_num_muteces, "lock.first_mutex=%u out of range: [0,%u]", lock.first_mutex, genome_num_muteces);
    ASSERT (lock.last_mutex  >= 0 && lock.last_mutex  <= genome_num_muteces, "lock.last_mutex=%u out of range: [0,%u]", lock.last_mutex, genome_num_muteces);
