...
static void flags_set_optimize (rom optarg)
{
    ...
    ASSINP (n_dicts <= MAX_DICTS, "--optimize specified %u fields, beyond the maximum of %u", n_dicts, MAX_FIELDS);
...
static void flags_set_dump_gz_block (rom optarg)
{
    ASSINP0 (optarg && str_get_int_range32 (optarg, 0, 0, 2 GB, &flag.dump_gz_block),
                "Usage: --dump_gz_block=𝑛 : 𝑛 is the 0-based gz block within a BGZF file");
...
{
    flag.explicit_head = true; // for testing conflict
...
static void flags_set_dict_(rom optarg, rom option_name, DictId *flag_with_dict)
{
    if (optarg) 
        *flag_with_dict = dict_id_make (optarg, strlen (optarg), DTYPE_PLAIN); 
        ...
        ABORTINP ("--%s requires a dict_id argument", &option_name[6]); // skip the prefix "&flag."
}
#define flags_set_dict(optarg, flag_with_dict) flags_set_dict_((optarg), #flag_with_dict, flag_with_dict);
...
static void flags_set_explicit_bgzf (rom level_str)
{
    if (level_str && !strcmp (level_str, "exact"))
        flag.bgzf = BGZF_EXACT; 
    
    else if (level_str && !strcmp (level_str, "exact-strict"))
        flag.bgzf = BGZF_EXACT_STRICT; 
    
    else {
        int32_t level;
        ASSINP (level_str && str_get_int_range32 (level_str, strlen (level_str), 0, MAX_FLAG_BGZF, &level),
                "--bgzf expects a value between 0 (no gz-recompression) and %u (best, yet slowest, gz-recompression), or \"exact\" (without the quotes) or \"exact-strict\". %s", 
                MAX_FLAG_BGZF, WEBSITE_GENOUNZIP_BGZF);
        flag.bgzf = level;
    }
...
static void flags_set_make_reference (rom optarg)
{
    if (!optarg) {                     
        flag.make_reference = MAKE_REF_DEFAULT;
        flag.default_make_ref = true;
        return;
    }
    
    for (int size=MAKE_REF_TINY; size <= MAKE_REF_MINIMAL; size++)
        if (!strcmp (optarg, make_ref_size_name (size))) {
            flag.make_reference = size;
            return;
        }

    ABORTINP ("Invalid --make-reference argument. Use: minimal, tiny, small, medium, large (default: medium). %s", WEBSITE_REFERENCE);
}

static void flag_set_telemetry (rom optarg)
{
    if (!optarg) 
        flag.telemetry = TELEMETRY_SEND;
...
static void flag_set_show_vblocks (rom optarg)
{
    flag.show_vblocks = optarg ? task_by_name (optarg) : SHOW_VBLOCKS_ALL_TASKS;
}
...
static void flag_set_stats (StatsType stats_type, rom optarg)
{
    ...
    flag.show_stats = (ABS((int)flag.show_stats) == stats_type) ? -stats_type : stats_type;
...
static void flag_set_show_containers (rom optarg)
{
    if (!optarg)
        ...
        flag.show_containers = SHOW_CONTAINERS_ALL_VBs; 
        flag.show_containers_δ = dict_id_make (optarg, strlen (optarg), DTYPE_PLAIN); // specific field
    }
}
...
{
START_TIMER;    
    ASSERT ((rom)&flag.flag_after_bits - (rom)&flag == 8, "size of first 64 bits=%u > 8", (int)((rom)&flag.flag_after_bits - (rom)&flag )); // if a multi-bit field straddles a byte boundary, or if there are too many bits (note: _Static_assert didn't calculate bitfields correctly)

    #include "flags_assign.c" // generated by flags_assign.sh (fint is a struct on the sstack)

    // process command line options
    while (1) {
        #define _i  {"input",            required_argument, 0, 'i'                    }
        #define _I  {"input-size",       required_argument, 0, 'I'                    }
        #define _d  {"decompress",       no_argument,       &fint.command,        PIZ }
        #define _f  {"force",            no_argument,       &fint.force,            1 }
        #define _h  {"help",             optional_argument, 0, 140                    }
        #define _l  {"list",             no_argument,       &fint.list,             1 }
        ...
        #define _qq {"no-tip",           no_argument,       &fint.no_tip,           1 } // note: supported in genocat/unzip as well, in case called from ref_fasta_to_ref
        #define _Q  {"noisy",            no_argument,       &fint.noisy,            1 }
        #define _q  {"quiet",            no_argument,       &fint.quiet,            1 }
        #define _DL {"replace",          no_argument,       &fint.replace,          1 }
        #define _nb {"no-bgzf",          no_argument,       &fint.no_bgzf,          1 }
        #define _nc {"no-cache",         no_argument,       &fint.no_cache,         1 }
        #define _nu {"no-upgrade",       no_argument,       &fint.no_upgrade,       1 }
        #define _hc {"hold-cache",       required_argument, 0, 145                    } // undocumented
        #define _V  {"version",          no_argument,       &fint.command,    VERSION }
        #define _z  {"bgzf",             required_argument, 0, 'z'                    }
        #define _zr {"cram",             no_argument,       &fint.out_dt,     DT_CRAM }
        #define _zR {"CRAM",             no_argument,       &fint.out_dt,     DT_CRAM }
        #define _zb {"bam",              no_argument,       &fint.out_dt,      DT_BAM }
        #define _zB {"BAM",              no_argument,       &fint.out_dt,      DT_BAM }
        #define _zs {"sam",              no_argument,       &fint.out_dt,      DT_SAM }
        #define _zS {"SAM",              no_argument,       &fint.out_dt,      DT_SAM }
        #define _zc {"bcf",              no_argument,       &fint.out_dt,      DT_BCF }
        #define _zC {"BCF",              no_argument,       &fint.out_dt,      DT_BCF }
        #define _zv {"vcf",              no_argument,       &fint.out_dt,      DT_VCF }
        #define _zV {"VCF",              no_argument,       &fint.out_dt,      DT_VCF }
        #define _za {"fasta",            no_argument,       &fint.out_dt,    DT_FASTA }
        #define _zA {"FASTA",            no_argument,       &fint.out_dt,    DT_FASTA }
        #define _zq {"fastq",            no_argument,       &fint.out_dt,    DT_FASTQ }
        #define _zQ {"FASTQ",            no_argument,       &fint.out_dt,    DT_FASTQ }
        #define _zf {"fq",               no_argument,       &fint.out_dt,    DT_FASTQ }
        #define _zF {"FQ",               no_argument,       &fint.out_dt,    DT_FASTQ }
        #define _9  {"optimize",         optional_argument, 0, '9',                   } // US spelling
        #define _88 {"optimise",         optional_argument, 0, '9',                   } // British spelling
        #define _m  {"md5",              no_argument,       &fint.md5,              1 }
        #define _t  {"test",             no_argument,       &fint.test,             1 }
        #define _Nt {"no-test",          no_argument,       &fint.no_test,          1 }
        #define _fa {"fast",             no_argument,       &fint.fast,             1 }
        #define _bs {"best",             no_argument,       &fint.best,             1 }
        #define _lm {"low-memory",       no_argument,       &fint.low_memory,       1 }
        #define _al {"add-line-numbers", no_argument,       &fint.add_line_numbers, 1 }
        #define _as {"add-seq",          no_argument,       &fint.add_seq,          1 }
        #define _Sd {"secure-DP",        no_argument,       &fint.secure_DP,        1 }
        #define _pe {"pair",             no_argument,       &fint.pair,        PAIRED } 
        #define _Np {"not-paired",       no_argument,       &fint.not_paired,       1 }
        ...
        #define _Rg {"gpos",             no_argument,       &fint.gpos,             1 }
        ...
        #define _x  {"index",            no_argument,       &fint.ext_indexing,     1 }
        #define _D  {"subdirs"  ,        no_argument,       &fint.subdirs,          1 }
        ...
        #define _G  {"drop-genotypes",   no_argument,       &fint.drop_genotypes,   1 }
        #define _H1 {"no-header",        no_argument,       &fint.no_header,        1 }
        #define _1  {"header-one",       no_argument,       &fint.header_one,       1 }
        #define _H0 {"header-only",      no_argument,       &fint.header_only,      1 }
        #define _H2 {"seq-only",         no_argument,       &fint.seq_only,         1 }
        #define _H3 {"qual-only",        no_argument,       &fint.qual_only,        1 }
        #define _H4 {"qname-only",       no_argument,       &fint.qname_only,       1 }
        #define _iu {"bases",            required_argument, 0, 24                     }
        #define _GT {"GT-only",          no_argument,       &fint.gt_only,          1 }
        #define _Gt {"gt-only",          no_argument,       &fint.gt_only,          1 }
        #define _So {"snps-only",        no_argument,       &fint.snps_only,        1 }
        #define _Io {"indels-only",      no_argument,       &fint.indels_only,      1 }
        #define _ds {"downsample",       required_argument, 0, 9                      }
        #define _PG {"no-PG",            no_argument,       &fint.no_pg,            1 }
        #define _pg {"no-pg",            no_argument,       &fint.no_pg,            1 }
        #define _fs {"sequential",       no_argument,       &fint.sequential,       1 }  
        #define _rG {"activate",         no_argument,       &fint.do_activate,      1 }
        #define _rg {"register",         optional_argument, 0, 28                     }  // legacy option
        #define _sL {"show-lines",       no_argument,       &fint.show_lines,       1 } 
        #define _ss {"stats",            optional_argument, 0, 'w',                   } 
        #define _SS {"STATS",            optional_argument, 0, 'W'                    } 
        #define _lc {"list-chroms",      no_argument,       &fint.show_contigs,     1 } // identical to --show-dict=CHROM 
        #define _lh {"chroms",           no_argument,       &fint.show_contigs,     1 } // identical to --show-dict=CHROM 
        #define _lH {"contigs",          no_argument,       &fint.show_contigs,     1 } 
        ...
        #define _sa {"show-alleles",     no_argument,       &fint.show_alleles,     1 }
        #define _st {"show-time",        optional_argument, 0, 1                      } 
        #define _sm {"show-memory",      no_argument,       &fint.show_memory,      1 } 
        #define _sh {"show-headers",     optional_argument, 0, 10                     } 
        #define _si {"show-index",       no_argument,       &fint.show_index,       1 } 
        #define _se {"show-regions",     no_argument,       &fint.show_regions,     1 } 
        #define _sG {"show-ranges",      no_argument,       &fint.show_ranges,      1 } 
        #define _Si {"show-ref-index",   no_argument,       &fint.show_ref_index,   1 } 
        #define _Sh {"show-ref-hash" ,   no_argument,       &fint.show_ref_hash,    1 } 
        #define _sr {"show-gheader",     optional_argument, 0, 135                    }  
        #define _SR {"show-reading-list",no_argument,       &fint.show_reading_list,1 }  
        #define _UM {"show-seg-summary", optional_argument, 0, 'v'                    } 
        #define _DT {"show-data-type",   no_argument,       &fint.show_data_type,   1 }  
        #define _sT {"show-threads",     no_argument,       &fint.show_threads,     1 }  
        #define _sS {"show-sec-gencomp", no_argument,       &fint.show_sec_gencomp, 1 }  
        #define _sN {"show-scan",        no_argument,       &fint.show_scan,        1 }  
        #define _wM {"show-wrong-MD",    no_argument,       &fint.show_wrong_md,    1 }  
        #define _wm {"show-wrong-XG",    no_argument,       &fint.show_wrong_xg,    1 }  
        #define _WM {"show-wrong-XM",    no_argument,       &fint.show_wrong_xm,    1 }  
        #define _WB {"show-wrong-XB",    no_argument,       &fint.show_wrong_xb,    1 }  
        #define _WT {"show-TLEN-pred",   no_argument,       &fint.show_tlen_pred,   1 }  
        #define _su {"show-uncompress",  no_argument,       &fint.show_uncompress,  1 }  
        #define _so {"show-compress",    no_argument,       &fint.show_compress,    1 }  
        #define _gz {"show-gz-uncomp",   no_argument,       &fint.show_gz_uncomp,   1 }  
        #define _sv {"show-vblocks",     optional_argument, 0, 141                    }  
        #define _sO {"show-txt-offsets", no_argument,       &fint.show_txt_offsets, 1 }
        #define _ai {"analyze-insertions", no_argument,     &fint.analyze_ins,      1 }  
        #define _ov {"one-vb",           required_argument, 0, 8                      }  
        #define _R1 {"R1",               no_argument,       &fint.one_component,    1 }  // comp_i+1
        #define _R2 {"R2",               no_argument,       &fint.one_component,    2 }  
        #define _RX {"R",                required_argument, 0, 146                    }  
        #define _sR {"show-reference",   no_argument,       &fint.show_reference,   1 }  // show-reference, show-ref-contigs, show-ref-iupacs, show-filename kept from back-compatability.
        #define _pR {"print-reference",  no_argument,       &fint.show_reference,   1 }  
        #define _sC {"show-ref-contigs", no_argument,       &fint.show_ref_contigs, 1 }  
        #define _pC {"print-ref-contigs",no_argument,       &fint.show_ref_contigs, 1 }  
        #define _rI {"show-ref-iupacs",  no_argument,       &fint.show_ref_iupacs,  1 }  
        #define _pI {"print-ref-iupacs", no_argument,       &fint.show_ref_iupacs,  1 }  
        #define _sn {"show-filename",    no_argument,       &fint.show_filename,    1 }  
        #define _pn {"print-filename",   no_argument,       &fint.show_filename,    1 }  
        #define _rA {"show-chrom2ref",   no_argument,       &fint.show_chrom2ref,   1 }  
        #define _rS {"show-ref-seq",     no_argument,       &fint.show_ref_seq,     1 }  
        #define _cn {"show-containers",  optional_argument, 0, 132                    }  
        #define _cN {"show-stack",       no_argument,       &fint.show_stack,       1 }
        #define _dd {"debug",            no_argument,       &fint.debug,            1 }
        #define _dg {"debug-seg",        optional_argument, 0, 133                    }  
        #define _hC {"show-txt-contigs", no_argument,       &fint.show_txt_contigs, 1 }
        #define _sI {"show-is-set",      required_argument, 0, '~',                   }  
        #define _sA {"show-aliases",     no_argument,       &fint.show_aliases,     1 }  
        #define _sB {"show-sag",         optional_argument, 0, 136                    }  
        #define _sP {"show-depn",        no_argument,       &fint.show_depn,        1 }  
        #define _sc {"show-codec",       optional_argument, 0, 159                    }  
        #define _Sc {"show-cache",       no_argument,       &fint.show_cache,       1 }  
        #define _AL {"show-aligner",     no_argument,       &fint.show_aligner,     1 }  
        #define _Ad {"debug-aligner",    no_argument,       &fint.debug_aligner,    1 }  
        #define _sb {"show-bgzf",        no_argument,       &fint.show_bgzf,        1 }
        #define _IS {"show-isizes",      no_argument,       &fint.show_isizes,      1 }
        #define _Sb {"show-gz",          no_argument,       0, SHOW_GZ                }
        #define _Dz {"dump-gz-block",    required_argument, 0, DUMP_GZ_BLOCK          }
        #define _Ie {"is-exactable",     no_argument,       &fint.show_is_exactable,1 }
        #define _s5 {"show-digest",      no_argument,       &fint.show_digest,      1 }
        #define _S5 {"log-digest",       no_argument,       &fint.log_digest,       1 }
        #define _s6 {"show-plan",        no_argument,       &fint.show_recon_plan,  1 }
        #define _sM {"show-mutex",       optional_argument, 0, 4                      }
        #define _dS {"seg-only",         no_argument,       &fint.seg_only,         1 }  
        #define _bS {"show-bam",         no_argument,       &fint.show_bam,         1 }  
        #define _iS {"show-bai",         optional_argument, 0, 11                     }  
        #define _tS {"show-tbi",         optional_argument, 0, 11                     }  // synonym
        #define _xt {"xthreads",         no_argument,       &fint.xthreads,         1 }  
        #define _dm {"debug-memory",     optional_argument, 0, 12                     }  
        #define _dp {"show-tasks",       no_argument,       &fint.show_tasks,       1 }  
        #define _dv {"debug-valgrind",   no_argument,       &fint.debug_valgrind,   1 }  
        #define _dA {"debug-bai",        no_argument,       &fint.debug_bai,        1 }          
        #define _TR {"debug-tar",        no_argument,       &fint.debug_tar,        1 }  
        #define _dL {"debug-LONG",       no_argument,       &fint.debug_LONG,       1 }  
        #define _dD {"show-qual",        no_argument,       &fint.show_qual,        1 }  
        #define _dq {"debug-qname",      no_argument,       &fint.debug_qname,      1 }  
        #define _dB {"show-buddy",       no_argument,       &fint.show_buddy,       1 }  
        #define _dt {"debug-threads",    no_argument,       &fint.debug_threads,    1 }  
        #define _dw {"debug-stats",      no_argument,       &fint.debug_stats,      1 }  
        #define _gn {"debug-generate",   no_argument,       &fint.debug_generate,   1 }  
        #define _dM {"debug-dyn-int",    no_argument,       &fint.debug_dyn_int,    1 }  
        #define _dr {"debug-recon-size", no_argument,       &fint.debug_recon_size, 1 }  
        #define _dR {"debug-read-ctxs",  no_argument,       &fint.debug_read_ctxs,  1 }  
        #define _dP {"debug-sag",        no_argument,       &fint.debug_sag,        1 }  
        #define _dG {"debug-gencomp",    no_argument,       &fint.debug_gencomp,    1 }  
        #define _dN {"no-gencomp",       no_argument,       &fint.no_gencomp,       1 }  
        #define _dF {"force-gencomp",    no_argument,       &fint.force_gencomp,    1 }  
        #define _RR {"force-reread" ,    no_argument,       &fint.force_reread,     1 }  
        #define _DF {"force-deep",       no_argument,       &fint.force_deep,       1 }  
        #define _fP {"force-PLy",        no_argument,       &fint.force_PLy,        1 }  
        #define _nS {"no-splice",        no_argument,       &fint.no_splicing,      1 }  
        #define _nF {"no-faf",           no_argument,       &fint.no_faf,           1 }  
        #define _nI {"no-interleaved",   no_argument,       &fint.no_interleaved,   1 }  
        ...
        #define _fl {"show-flavor",      optional_argument, 0, SHOW_FLAVOR            }  
        ...
        #define _sp {"debug-split",      no_argument,       &fint.debug_split,      1 }
        #define _Du {"debug-upgrade",    no_argument,       &fint.debug_upgrade,    1 }
        #define _De {"debug-expiration", no_argument,       &fint.debug_expiration, 1 }
        #define _np {"show-snips",       no_argument,       &fint.show_snips,       1 }
        #define _Ds {"telemetry",        optional_argument, 0, 138                    }
        #define _DD {"debug-debug",      no_argument,       &fint.debug_debug,      1 }
        #define _RC {"recover",          no_argument,       &fint.recover,          1 }
        #define _NE {"no-eval",          no_argument,       &fint.no_eval,          1 }
        ...
        static rom short_options[NUM_EXE_TYPES] = { 
            [EXE_GENOZIP  ] = "z:i:I:cdfhLqQt^Vz:m@:o:p:B:9wWvFe:E:C:23K:T:DbXxA:", // includes some genounzip options to be used in combination with -d
            [EXE_GENOUNZIP] = "cz:fhLqdQt^V@:u:o:p:me:wWxT:D",
            [EXE_GENOCAT  ] = "z:hLV@:dp:qQ1r:R:s:H1Go:fg:e:E:wWxvk:K:n:yT:D",
            [EXE_GENOLS   ] = "hLVp:qfblT:"
        };

        int option_index = -1;
        int c = getopt_long (argc, argv, short_options[exe_type], long_options, &option_index);
        ...
        switch (c) {
            case HELP          : flag.help = optarg;        goto verify_command;        // 150
            case LICENSE       : flag.lic_param = optarg;   goto verify_command;        // 'L'
            case SHOW_BAI      : bai_set_show_bai (optarg); goto verify_command;        // 11
            case BIOPSY_BYTES  : biopsy_bytes_init (optarg);goto verify_command;        // 148
            case GENERATE_IL1M :                            goto verify_command;        // 153
            case SHOW_FLAVOR   : flag.show_flavor = optarg; goto verify_command;        // 156
            case SHOW_GZ       :                            goto verify_command;        // 157
            case DUMP_GZ_BLOCK : flags_set_dump_gz_block (optarg); goto verify_command; // 158
            case PIZ           : if (is_genozip) exe_type = EXE_GENOUNZIP; /*needed due to incompleteness of set_ext_type*/ // 'd'
                                 goto verify_command;
            case VERSION       :                                                        // 'V'
verify_command:
                ASSINP0 (flag.command == NO_COMMAND || flag.command==c, "Conflicting command line options"); 
                flag.command=c; 
                break;
            ...
            case 'v' : flag.show_seg_summary = 1    ; break;
            ...
            case 'x' : flag.ext_indexing  = 1       ; break;
            ...
            case 2   : flags_set_dict (optarg, &flag.show_b250_δ); break; 
            case 3   : if (optarg) flag.show_one_dict = optarg;
                       else        flag.show_dict = 1;
                       break;
            case 4   : flag.show_mutex    = optarg ? optarg : (char*)1; break;
            case 5   : flags_set_dict (optarg,  &flag.dump_b250_δ); break;
            case 6   : flags_set_dict (optarg,  &flag.dump_local_δ); break;
            case 150 : flag.show_singletons_δ = dict_id_make (optarg, strlen (optarg), DTYPE_PLAIN); break;
            ...
            case 131 : flags_set_dict (optarg, &flag.show_huffman_δ); break;
            ...
            case 138 : flag_set_telemetry (optarg)   ; break;
            case 139 : flag_set_show_deep (optarg)   ; break;
            case 141 : flag_set_show_vblocks (optarg); break;
            ...
            case 159 : flags_set_dict (optarg, &flag.show_codec_δ); break;
            case 160 : flags_set_make_reference (optarg); break;
            case 0   : break; // a long option that doesn't have short version will land here - already handled so nothing to do
                 
            default  : // unrecognized option 
                ABORTINP ("See: %s/%s\n", GENOZIP_URL, global_cmd); // add to the "unknown option" message already printed by the library
        }
    }

    FLAGS_ASSIGN_BOOL // assign to bool members of flag (defined in flags_assign.c)
    ...
    // set commands which are not already set in the switch statement ^
    flag.command = (flag.command != NO_COMMAND)      ? flag.command 
                 : is_genols                         ? LIST
...
static void flags_test_conflicts (unsigned num_files /* optional */)
{
    ...
    if (IS_ZIP || IS_PIZ) {
        ...
        CONFLICT (flag.dump_b250_δ.num, flag.dump_local_δ.num, "--dump-b250", "--dump-local");
    }
    ...
    if (IS_ZIP) {
        ...
        CONFLICT (flag.md5,             flag.truncate,          OT("md5", "m"),        "--truncate"); // bug 1239
        ...
        CONFLICT (flag.test,            flag.show_is_exactable, "--is-exactable",      OT("test", "t"));
        ...
        CONFLICT (flag.ext_indexing,    flag.make_reference,    "--make-reference",    OT("index", "x"));
        CONFLICT (flag.ext_indexing,    flag.no_index,          "--no-index",          OT("index", "x"));
        ...
        CONFLICT (flag.deep,            flag.show_is_exactable, OT("deep", "3"),       "--is-exactable");
        ...
        CONFLICT (flag.deep,            IS_BIOPSY_BYTES,        OT("deep", "3"),       "--biopsy-bytes");
        CONFLICT (flag.deep,            flag_has_biopsy_line,   OT("deep", "3"),       "--biopsy-line");
        CONFLICT (flag.bam_assist,      flag.show_is_exactable, OT("bamass", "A"),     "--is-exactable");
        ...
        CONFLICT (flag.bam_assist,      IS_BIOPSY_BYTES,        OT("bamass", "A"),     "--biopsy-bytes");
        CONFLICT (flag.bam_assist,      flag_has_biopsy_line,   OT("bamass", "A"),     "--biopsy-line");
        CONFLICT (flag.bam_assist,      flag.subdirs,           OT("bamass", "A"),     OT("subdirs", "D"));
        CONFLICT (flag.pair,            tar_zip_is_tar(),       OT("pair", "2"),       "--tar");
        CONFLICT (flag.pair,            flag.show_is_exactable, OT("pair", "2"),       "--is-exactable");
    ...
    if (IS_PIZ) {
        ...
        // options that require absence of --reference 
            ...
            ASSINP (!flag.show_reference, "%s is incompatible with --print-reference. To use --print-reference on a reference file, omit --reference", OT("reference", "e"));
...
#ifndef DEBUG
    ASSINP0 (!flag.debug_aligner, "--debug-aligner works only in genozip-debug"); 
#endif
}
...
#define FLAG_ONLY_FOR_3DTs(dt1, dt2, dt3, F, Fname)\
    ASSINP0 (!flag.F || (dt == DT_##dt1) || (dt == DT_##dt2) || (dt == DT_##dt3) || (DT_##dt1 == DT_SAM && dt == DT_BAM), \
                "--" Fname " is only supported for " #dt1 ", " #dt2 " and " #dt3 " files");
...
static void flags_piz_verify_dt_specific (DataType dt)
{
    ...
    // REFERENCE
    FLAG_NOT_FOR_DT (REF,          show_reference,"print-reference");
}

static void flag_zip_verify_ref_indicated (rom option_name)
{
    if (!flag.reference) {
        rom env = getenv (GENOZIP_REFERENCE);

        ASSINP (env, "For %s, you must specficy a reference file with --reference or --REFERENCE or $%s", 
                option_name, GENOZIP_REFERENCE);

        ASSINP (!file_is_dir (env), 
                "$%s=%s is a directory. Set $%s to a reference file name or override with --reference", 
                GENOZIP_REFERENCE, env, GENOZIP_REFERENCE);
    }
}
...
static void flags_zip_verify_pair_rules (unsigned num_files, rom *filenames)
{
    ...
    ASSINP (!flag.biopsy && !flag_has_biopsy_line, "%s cannot be used with --biopsy or --biopsy-line, please take a biopsy from each file seperately. "_TIP"Use 'genocat --show-txt-offsets' to get VB location in txt, then use 'genozip --biopsy-bytes'", OT("pair", "2"));

    ASSINP (!IS_BIOPSY_BYTES, "%s cannot be used with --biopsy-bytes", OT("pair", "2"));
    ...
    flag_zip_verify_ref_indicated (OT("pair", "2"));

    // verify all are fastq or all are fasta
    bool has_fastq=false, has_fasta=false;
    for (unsigned i=0; i < num_files; i++) {
        DataType dt = txtfile_zip_get_file_dt (filenames[i]);
        has_fasta |= (dt == DT_FASTA);
        has_fastq |= (dt == DT_FASTQ);
        ASSINP (dt == DT_FASTQ /*|| dt == DT_FASTA (bug 1218)*/, "when using %s, all input files are expected to be FASTQ files, but %s is not", OT("pair", "2"), filenames[i]);
    }
    ASSINP (has_fastq != has_fasta, "when using %s, either FASTQ or FASTA are accepted, but not mixed", OT("pair", "2"));
...
static void flags_zip_verify_deep_rules (unsigned num_files, rom *filenames)
{
    ...
    for (int i=0; i < num_files; i++) {
        ...
        if (dt != DT_FASTQ && dt != DT_FASTA && dt != DT_BAM && dt != DT_SAM) goto validation;
    ...
    flag.deep_num_fastqs = n_dt[DT_FASTQ] + n_dt[DT_FASTA]; // 1 byte (overflow tested below)

validation:
    ASSINP (n_dt[DT_SAM] + n_dt[DT_BAM] == 1 && 
            n_dt[DT_FASTQ] + n_dt[DT_FASTA] >= 1 && 
            (n_dt[DT_SAM] + n_dt[DT_BAM] + n_dt[DT_FASTQ] == num_files || n_dt[DT_SAM] + n_dt[DT_BAM] + n_dt[DT_FASTA] == num_files),
            "when using %s, expecting one SAM/BAM/CRAM and one or more FASTQ or FASTA files", OT("deep", "3"));

    ASSINP (n_dt[DT_FASTQ] + n_dt[DT_FASTA] <= MAX_NUM_TXT_FILES_IN_ZFILE-1, "Genozip currently supports Deep compression with up to %u FASTQ (or FASTA) files, but you specified %u. If you need us to increase this limit, please contact " EMAIL_SUPPORT ".",
            MAX_NUM_TXT_FILES_IN_ZFILE-1, n_dt[DT_FASTQ] + n_dt[DT_FASTA]);

    flag_zip_verify_ref_indicated (OT("deep", "3"));
    ...
    if (file_exists (filenames[0])) { // if doesn't exist, we will error elsewhere
        double bam_size_gb = (double)file_get_size (filenames[0]) / (double)(1 GB);
        double ram_gb = arch_get_physical_mem_size();
        ASSERTW (ram_gb >= bam_size_gb * 2, _FYI "--deep requires a signifcant amount of RAM - usually between 1 and 2 times the size of the BAM file. Your BAM file is %.1f GB while your RAM has only %.1f GB - memory might not be sufficient\n",
                 bam_size_gb, ram_gb);
    }
}
...
static void flags_zip_verify_bamassist_rules (unsigned num_files, rom *filenames)
{
    ...
    for (int i=0; i < num_files; i++) {
        rom basename = filename_base (filenames[i], false, 0, 0, 0);
        DataType dt = txtfile_zip_get_file_dt (basename);
        ASSINP (dt == DT_FASTQ || dt == DT_FASTA, "--bamass can only operate on FASTQ and FASTA files, but \"%s\" is not one", filenames[i]);
        FREE (basename);
    }

    ASSINP (num_files <= MAX_NUM_COMPS, "You listed %u files, but --bamass supports up to %u", num_files, MAX_NUM_COMPS);
...
void flags_update (unsigned num_files, rom *filenames)
{
    ...
    if (flag.show_dict || flag.show_b250 || flag.show_headers || flag.show_threads || flag.show_bgzf || flag.show_mutex || flag.show_containers || flag.show_stack ||
        ...
        flag.show_sec_gencomp || flag.show_recon_plan || flag.show_reading_list || flag.show_scan || flag.show_isizes || flag.show_uncompress ||
        flag.show_deep == SHOW_DEEP_ALL)
    ...
    flag.dont_load_ref_file |= is_genocat &&
        (flag.show_stats || flag.show_dict || flag.show_b250 || flag.show_contigs || flag.show_one_dict ||
         flag.dump_b250_δ.num || // all other sections (except CHROM) are blocked from reading in piz_default_skip_section
    ...
    if (flag.log_filename) {
        if (!(info_stream = fopen (flag.log_filename, "a"))) // cannot use ASSERT here bc no info_stream yet...
            fprintf (stderr, _FYI "Failed to open log file %s for writing: %s", flag.log_filename, strerror (errno));
        else
            iprintf ("\n%s\n%s\n", str_time().s, B1STc(command_line));
    }
    ...
    flag.no_tip |= flag.quiet || (flag.test_i && !flag.noisy) || !isatty(0) || !isatty (1) ||
                   flag.show_bam || flag.show_bai || zip_is_biopsy;
...
void flags_update_zip_one_file (void)
{
    ...
    if (flag.make_reference) {
        ...
        if (flag.show_stats) flag.show_ref_hash = true; // --stats implies --show-ref-hash
    }
    ...
    if (flag.sendto) TIP0 ("Compressing using --sendto means that this file can only be uncomrpessed by its intended recipient.");
    ...
    flag.test |= !flag.no_test && !flag.make_reference && !zip_is_biopsy &&
                 !flag.show_bam && !flag.show_bai && !flag.seg_only && 
    ...
    if (!Z_DT(FASTQ)) 
        flag.no_splicing = true; // splicing not implemented yet for SAM/BAM (bug 1273)
...
bool flags_is_genocat_global_area_only (void) // genocat exits after reading global area
{
    return is_genocat &&
        (flag.show_stats || flag.show_dict || flag.show_contigs || flag.show_one_dict ||
         flag.show_index || flag.show_counts_δ.num || IS_SHOW_HEADERS ||
         flag.show_reference || flag.show_ref_contigs || flag.show_ranges || 
         flag.show_ref_index || flag.show_ref_hash || flag.show_chrom2ref || flag.show_huffman_δ.num ||
         flag.show_ref_seq || flag.show_aliases || flag.show_gheader || flag.show_data_type ||
         flag.show_txt_offsets ||
         flag.show_b250 || flag.show_b250_δ.num || // in genocat, zfile_show_b250_only is called instead of piz loop 
         (Z_DT(REF) && flag.show_ref_iupacs));    
}

static void flags_piz_set_flags_for_deep (void)
{
    if (is_genocat && flag.explicit_out_dt && OUT_DT(FASTA) && segconf.fasta_as_fastq)
    ...
    // flag.explicit_out_dt is set only when opening the TXT file, but we error here before.
        ...
        ASSINP (sam || bam || cram || fastq || flags_is_genocat_global_area_only(), 
                "This file was compressed with --deep. Therefore, genocat requires one of these options: --R1, --R2, --R=𝑛, --interleaved, --fast%c, --sam, --bam, --one-vb=𝑛",
                segconf.fasta_as_fastq ? 'a' : 'q');
...
void flags_update_piz_no_ref_file (void)
{
    bool never_need_ref = is_genocat && 
        (flags_writer_counts() || flag.qname_only || 
         IS_SHOW_HEADERS || // note: when calling from genocat_show_headers flag.show_headers==false so we use command instead
         flag.show_stats || flag.show_dict || flag.show_contigs || flag.show_one_dict || 
         flag.show_b250 || flag.show_b250_δ.num ||
         flag.show_txt_offsets ||
         flag.show_index || flag.show_data_type || flag.show_huffman_δ.num || flag.show_isizes ||
...
void flags_update_piz_one_z_file (int z_file_i /* -1 if unknown - called form file_open_txt_write */)
{
    ...
    ASSINP (!flag.test || z_file->z_flags.has_digest, 
            "--test cannot be used with %s, as it was compressed without a digest. %s", z_name, WEBSITE_DIGEST);
    ...
    ASSINP (!is_genounzip || !flag.out_filename || z_file->num_txt_files == 1, 
            ...
            "\n- Use --output with the target directory name (" _TIP "end name with / for genozip to mkdir if needed)"
            "\n- Use genocat with --bam, --fastq, --R1, --interleaved etc to uncompress a single component. %s",
    ...
    // translator limitations (before flags_piz_verify_dt_specific)
    ...
    if (Z_DT(FASTA)) {
        ...
        ASSINP (flag.one_component <= z_file->num_components, 
                "--R=%u is not supported for %s because it contains only %u FAST%c component%s", 
                flag.one_component - n_sam_comps, z_name, z_file->num_components - n_sam_comps,
                QorA, s_or_nil(z_file->num_components - n_sam_comps));
        ...
        if (OUT_DT(BAM) || OUT_DT(CRAM)) 
            ASSINP (flag.no_header != 1, "Using --no-header not supported when outputting %s. " _TIP "To output as SAM, use --sam.", dt_name (flag.out_dt));
    }
    ...
    if (flag.out_filename && is_genounzip) {
        ...
        WARN_IF (flag.out_dt != out_dt_by_filename, "genounzip is reconstructing a %s file, is this what you intended? Asking because filename is \"%s\".%s",
                 ...
                 ? "\n" _TIP "To convert between file types, use genocat." : ""); // show tip only if relevant
    }
    ...
    ASSINP (!flag.no_writer || !flag.ext_indexing, "%s cannot be used because no data is written", OT("index", "x"));
    
    if (flag.ext_indexing && bai_is_native_indexing_supported (flag.out_dt))
    ...
    flag.genocat_no_dicts = is_genocat &&
        (flag.show_stats || flag.show_index || flag.show_counts_δ.num || // note: we need dicts for dump_b250 as we need to reconstruct
         flag.show_reference || flag.show_ref_contigs || 
         flag.show_ref_index || flag.show_ref_hash || flag.show_chrom2ref || flag.show_isizes ||
    ...
    ASSINP0 (!flag.test || !flag.piz_txt_modified, "--test cannot be used when other flags specify data modification. " WEBSITE_DIGEST);

    ASSINP (!flag.piz_txt_modified || !IS_PIZ_EXACT, 
            "Cannot use --bgzf=exact%s, because other command line options might cause output file to differ from original file", 
            (flag.bgzf == BGZF_EXACT_STRICT ? "-strict" : ""));
    ...
    ASSINP (!flag.grep || !DTPO(is_binary), "--grep is not supported when outputting %s data%s", 
            dt_name (flag.out_dt), OUT_DT(BAM) || OUT_DT(CRAM) ? ". " _TIP "add --sam": "");
    ...
    ASSINP (VER(12) || (flag.lines_first == NO_LINE && !flag.tail && !flag.downsample),
            ...
            z_name, z_file->genozip_ver.major);
...
void flags_store_command_line (int argc, char **argv)
{
    ...
    for (int i=0; i < argc; i++) {
        ...
        // drop undocumented internal flags
        else if (!strcmp (argv[i], "--restarted"))
            continue;
            ...
            bool need_quotes = !!strpbrk (argv[i], " \t\n*?[]<>|$`&;()#!~");

            if (need_quotes) buf_append_string (evb, &command_line, "\"");
        ...
        if (!i) {
            ...
            str_replace_letter (STRb(debugger_params), '\\', '/');
        }
        else {
