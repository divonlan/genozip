...
static void flags_set_dict_ (rom optarg, rom option_name, 
                             int *flag_without_dict, // NULL if dict is required
                             DictId *flag_with_dict)
{
    if (optarg) 
        *flag_with_dict = dict_id_make (optarg, strlen (optarg), DTYPE_PLAIN); 
    
    else if (flag_without_dict) 
        *flag_without_dict = 1;    

    else
        ABORTINP ("--%s requires a dict_id argument", &option_name[6]); // skip the prefix "&flag."
}
#define flags_set_dict(optarg, flag_without_dict, flag_with_dict) flags_set_dict_((optarg), #flag_with_dict, flag_without_dict, flag_with_dict);
...
{
    flag.show_vblocks = optarg ? task_by_name (optarg) : -1/*all tasks*/;
}
...
void flags_init_from_command_line (int argc, char **argv)
{
    ...
    while (1) {
        ...
        #define _zV {"VCF",              no_argument,       PADDED(out_dt),    DT_VCF }
        #define _za {"fasta",            no_argument,       PADDED(out_dt),  DT_FASTA }
        #define _zA {"FASTA",            no_argument,       PADDED(out_dt),  DT_FASTA }
        ...
        #define _x  {"index",            no_argument,       &flag.ext_indexing,     1 }
        ...
        #define _se {"show-regions",     no_argument,       &flag.show_regions,     1 } 
        ...
        #define _UM {"show-seg-summary", no_argument,       &flag.show_seg_summary, 1 } 
        ...
        #define _WT {"show-TLEN-pred",   no_argument,       &flag.show_tlen_pred,   1 }  
        ...
        #define _sO {"show-txt-offsets", no_argument,       &flag.show_txt_offsets, 1 }
        ...
        #define _Ad {"debug-aligner",    no_argument,       &flag.debug_aligner,    1 }  
        #define _sb {"show-bgzf",        no_argument,       &flag.show_bgzf,        1 }
        #define _IS {"show-isizes",      no_argument,       &flag.show_isizes,      1 }
        #define _Sb {"show-gz",          no_argument,       0, SHOW_GZ                }
        #define _Dz {"dump-gz-block",    required_argument, 0, DUMP_GZ_BLOCK          }
        #define _Ie {"is-exactable",     no_argument,       &flag.show_is_exactable,1 }
        ...
        #define _dp {"show-tasks",   no_argument,       &flag.show_tasks,   1 }  
        #define _dv {"debug-valgrind",   no_argument,       &flag.debug_valgrind,   1 }  
        #define _dA {"debug-bai",        no_argument,       &flag.debug_bai,        1 }          
        ...
        #define _gn {"debug-generate",   no_argument,       &flag.debug_generate,   1 }  
        #define _dM {"debug-dyn-int",    no_argument,       &flag.debug_dyn_int,    1 }  
        ...
        #define _nS {"no-splice",        no_argument,       &flag.no_splicing,      1 }  
        ...
        #define _dz {"no-lzma",          no_argument,       &flag.no_lzma,          1 }  
        #define _dI {"no-bai",           no_argument,       &flag.no_index,         1 }  
        #define _dT {"no-tbi",           no_argument,       &flag.no_index,         1 }  
        #define _SI {"skip-index",       no_argument,       &flag.skip_index,       1 }  
        ...
        #define _oe {"echo",             optional_argument, &flag.echo,             1 }
        ...
        #define _rs {"restarted",        no_argument,       &flag.restarted,        1 }
        #define _00 {0, 0, 0, 0                                                       }

        typedef const struct option Option;
        static Option genozip_lo[]   = { _lg, _i, _I, _d, _f, _h, _x, _D,    _L1, _L2, _q, _Q, _qq, _t, _Nt, _DL, _nb, _nc,_nu,  _V, _z,                                                                                 _m, _th,     _o, _p, _e, _E,                                                                       _H1,                                              _sL, _ss, _SS,           _sd, _sT,      _sN, _sb, _IS, _Sb, _Dz, _Ie, _lc, _lh, _lH,      _s7, _S7, _S0, _S8, _S9, _sa, _st, _sm, _sh, _si, _Si, _Sh, _sr,      _UM, _su,      _so, _gz, _sv, _sn, _pn, _ai,                    _B, _xt, _dm, _dp, _dL, _dD, _dq, _dB, _dt, _dw, _dM, _gn, _dr, _dR, _dP, _dG, _dN, _dF, _RR, _DF, _dQ, _fl, _dH, _Hh, _dO, _dC, _fQ, _fC, _fO, _fS, _fH, _fN, _dU, _dz, _dl, _dc, _dg,      _dh,_dS, _dI, _dT, _SI, _bS, _iS, _tS, _9, _88, _pe, _Np, _fa, _bs, _lm,                        _nh, _rg, _rG,                          _hC, _rA,           _rS, _me, _s5, _S5, _sM, _sA, _sB, _sP, _sc, _Sc, _AL, _Ad, _sI, _cn,                                    _s6,          _oe, _al, _as, _Lf, _dd, _T, _TT, _TL, _wM, _wm, _WM, _WB, _WT, _bi, _bB, _bl, _sk, _VV, _DV,      _Dh, _Ds, _sp, _Du, _De, _DD, _DP, _BA, _SH, _Dd, _ba,      _to, _ts,      _hc, _dv, _dA, _TR, _NE, _Sd, _St, _um,      _fP, _nF, _nS, _nI, _gg, _rs, _00 };
        ...
        switch (c) {
            ...
            case VERSION       :                                                        // 'V'
verify_command:
                ASSINP0 (command < 0 || command==c, "Conflicting command line options"); 
                command=c; 
                break;
            ...
            case 2   : flags_set_dict (optarg, &flag.show_b250, &flag.show_b250_δ); break; 
            case 3   : if (optarg) flag.show_one_dict = optarg;
                       else        flag.show_dict = 1;
                       break;
            case 4   : flag.show_mutex    = optarg ? optarg : (char*)1; break;
            case 5   : flags_set_dict (optarg, NULL, &flag.dump_b250_δ); break;
            case 6   : flags_set_dict (optarg, NULL, &flag.dump_local_δ); break;
            ...
            case 16  : flags_set_dict (optarg, NULL, &flag.show_counts_δ); break;
            case 155 : flags_set_dict (optarg, NULL, &flag.debug_huffman_δ); // fallthrough: debug_huffman implies show_huffman
            case 131 : flags_set_dict (optarg, NULL, &flag.show_huffman_δ); break;
            ...
            case 133 : flag.debug_seg=1;
                       flags_set_dict (optarg, NULL, &flag.debug_seg_δ); break;
            ...
            case 159 : flags_set_dict (optarg, &flag.show_codec, &flag.show_codec_δ); break;
    ...
    command = (command != NO_COMMAND)           ? command 
            : is_genols                         ? LIST
...
static void flags_zip_verify_deep_rules (unsigned num_files, rom *filenames)
{
    ...
    flag.deep_num_fastqs = n_dt[DT_FASTQ] + n_dt[DT_FASTA];
