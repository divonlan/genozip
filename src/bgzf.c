...
static FlagsBgzf bgzf_load_isizes (CompIType comp_i, bool show_only) 
{
    ...
    // if level discover failed)
    if (header->flags.bgzf.level == BGZF_COMP_LEVEL_UNKNOWN) 
        goto fallback;
...
FlagsBgzf bgzf_piz_calculate_bgzf_flags (CompIType comp_i, Codec src_codec)
{
    ...
    if (flag.test    || 
        ...
        bgzf_flags = bgzf_no_recompression; 
    
    // case: reconstructing BCF: piz sends VCF to bcftools in CODEC_NONE, and bcftools compressed by the level given in bgzf_flags
    else if (OUT_DT(BCF))
        bgzf_flags = (FlagsBgzf){ .library = BGZF_EXTERNAL_LIB, 
    ...
    // case: --bgzf=0 to 5
        ...
        bgzf_flags = recompression_template (flag.bgzf); // set to --bgzf command line value

        // if user specified --bgzf and --output - make sure output filename is .gz, .bam or .bcf
        ASSINP (flag.force || !flag.out_filename || bgzf_implied_by_out_filename || HAS_EXT(.bcf) || bgzf_flags.level==0, 
    ...
    // case: genocat to stdout without --bgzf: - no re-compression. 
        ...
        bgzf_flags = OUT_DT(BAM) ? bgzf_recompression_levels[0] : bgzf_no_recompression; // file_open_txt_write interprets level=0 as CODEC_BGZF without compression for BAM, and CODEC_NONE for other types 
    ...
    // case: genocat or genounzip out_filename and no --bgzf: - determine by file name (except BAM - bgzf regardless of filename)
        ...
        bgzf_flags = (bgzf_implied_by_out_filename || OUT_DT(BAM) || (!no_bgzf_implied_by_out_filename && !C(NONE))) ? bgzf_recompression_levels[BGZF_DEFAULT_LEVEL] : bgzf_no_recompression;
    ...
    if (flag.show_bgzf)
        iprintf ("comp_i=%u with src_codec=%s out_dt=%s: calculated bgzf_flags={%s, %d}\n",
                 comp_i, codec_name (src_codec), dt_name (flag.out_dt), bgzf_library_name (bgzf_flags.library, true), bgzf_flags.level);
...
void bgzf_piz_set_txt_file_bgzf_info (FlagsBgzf bgzf_flags, bytes codec_info)
{
    ...
    // sanity        
    ASSERT (txt_file->bgzf_flags.level >= 0 && txt_file->bgzf_flags.level <= BGZF_MAX_LEVEL, "txt_file->bgzf_flags.level=%u ∉ [0,%u]", 
            txt_file->bgzf_flags.level, BGZF_MAX_LEVEL);

    ASSERT (txt_file->bgzf_flags.library >= 0 && txt_file->bgzf_flags.library < NUM_BGZF_LIBRARIES, "txt_file->bgzf_flags.library=%u ∉ [0,%u]", 
...
static void bgzf_alloc_compressor (VBlockP vb, FlagsBgzf bgzf_flags)
{
    ...
    if (flag.show_bgzf)
        iprintf ("%s: initialized compressor %s level %u%s\n", VB_NAME,
