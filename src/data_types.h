...
typedef struct DataTypeProperties {
    ...
    uint8_t vb_1_not_representative;            // very beginning of might have data that is statistical different than the rest of the file (eg because close to the beginning of a chromosome). bit field: bits refer to DTYPE_FIELD, DTYPE_1, DTYPE_2
    ...
    int32_t (*unconsumed) (VBlockP, uint32_t first_i);  // called by main thread called by txtfile_get_unconsumed_to_pass_to_next_vb to get the length of unconsumed txt to pass to next vb. returns -1 if first_i is too high and it needs more data.
    ...
    void (*zip_after_segconf)(VBlockP vb);      // called by main thread after segconf has completed
    ...
    void (*zip_end_of_z)(void);            // called by main thread after writing global area
    ...
    void (*segconf_finalize)(VBlockP);          // called by Compute thread at the end of segconf, before seg_finalize
    ...
    void (*zip_comp_cb)(VBlockP, ContextP, SectionType, uint32_t); // called after compressing sections (B250 and LOCAL for now)
    ...
    void (*piz_header_init)(CompIType comp_i);  // called at the beginning of reconstruction of every txt header that is reconstructed
    ...
    void (*piz_after_vb_header)(VBlockP);       // called by main thread after reading VB_HEADER and before reading local/b250 sections from z_file
    ...
    void (*piz_vb_recon_init)(VBlockP);         // called by the compute thread after uncompressing and before reconstructing
    ...
    void (*piz_after_preproc_vb)(VBlockP);      // called by the main thread after joinig a preprocessing thread
    void (*piz_preproc_finalize)(Dispatcher dispatcher); // called by the main thread after all preprocessing VBs are joined
...
/*                 name         is_bin \n-end use_ref vb1_not txt_type   bin_type  sizeof_vb       sizeof_zip_dataline   txt_headr   hdr_contigs     1st  is_header_done          unconsumed          inspect_txt_header      is_data_type  zip_initialize          zip_after_segconf        zip_after_vbs,     zip_finalize         zip_end_of_z           zip_init_vb        zip_after_compute          zip_set_vb_header_specific        zip_set_txt_header_flags         zip_modify        seg_initialize          seg_txt_line          assseg_line          seg_is_big            seg_is_small          seg_finalize          segconf_finalize        zip_custom_merge     seg_modifies zip_comp_cb        zip_after_compress        stats_reallocate        zip_genozip_header        piz_genozip_header        piz_after_global_area   piz_preprocess                          piz_header_init        piz_initialize          piz_finalize         piz_after_vb_header        piz_init_vb          piz_vb_recon_init           piz_init_line       piz_after_recon             piz_process_recon         piz_after_preproc_vb      piz_preproc_finalize      piz_xtra_line_data      is_skip_section             reconstruct_seq            container_filter       container_cb              con_item_cb          num_special          special          num_trans        translators         line_name        dtype_names                             */ \
    [DT_VCF]   = { "VCF",       false, true,  true,   0b110,  DT_VCF,    DT_NONE,  vcf_vb_size,    vcf_vb_zip_dl_size,   HDR_MUST,   VCF_CONTIG_FMT, '#', vcf_is_header_done,     NULL,               vcf_inspect_txt_header, is_vcf,       vcf_zip_initialize,     NULL,                    vcf_zip_after_vbs, vcf_zip_finalize,    vcf_header_finalize,   vcf_zip_init_vb,   vcf_zip_after_compute,     vcf_zip_set_vb_header_specific,   vcf_zip_set_txt_header_flags,    vcf_zip_modify,   vcf_seg_initialize,     vcf_seg_txt_line,     NULL,                vcf_seg_is_big,       vcf_seg_is_small,     vcf_seg_finalize,     vcf_segconf_finalize,   NULL,                false,       NULL,              vcf_zip_after_compress,   NULL,                   vcf_zip_genozip_header,   vcf_piz_genozip_header,   NULL,                   NULL,                                   vcf_piz_header_init,   vcf_piz_initialize,     vcf_piz_finalize,    NULL,                      vcf_piz_init_vb,     vcf_piz_vb_recon_init,      vcf_reset_line,     NULL,                       NULL,                     NULL,                     NULL,                     NULL,                   vcf_piz_is_skip_section,    NULL,                      vcf_piz_filter,        vcf_piz_container_cb,     vcf_piz_con_item_cb, NUM_VCF_SPECIAL,     VCF_SPECIAL,     0,               {},                 "variant",       { "FIELD", "INFO",   "FORMAT", "BOTH" } }, \
    [DT_BCF]   = { "BCF",       true,  true,  true,   0b110,  DT_VCF,    DT_BCF,   vcf_vb_size,    vcf_vb_zip_dl_size,   HDR_MUST,   VCF_CONTIG_FMT, '#', NULL,                   NULL,               vcf_inspect_txt_header, NULL,         vcf_zip_initialize,     NULL,                    vcf_zip_after_vbs, vcf_zip_finalize,    vcf_header_finalize,   vcf_zip_init_vb,   vcf_zip_after_compute,     vcf_zip_set_vb_header_specific,   vcf_zip_set_txt_header_flags,    vcf_zip_modify,   vcf_seg_initialize,     vcf_seg_txt_line,     NULL,                vcf_seg_is_big,       vcf_seg_is_small,     vcf_seg_finalize,     vcf_segconf_finalize,   NULL,                false,       NULL,              vcf_zip_after_compress,   NULL,                   vcf_zip_genozip_header,   vcf_piz_genozip_header,   NULL,                   NULL,                                   vcf_piz_header_init,   vcf_piz_initialize,     vcf_piz_finalize,    NULL,                      vcf_piz_init_vb,     vcf_piz_vb_recon_init,      vcf_reset_line,     NULL,                       NULL,                     NULL,                     NULL,                     NULL,                   vcf_piz_is_skip_section,    NULL,                      vcf_piz_filter,        vcf_piz_container_cb,     vcf_piz_con_item_cb, NUM_VCF_SPECIAL,     VCF_SPECIAL,     0,               {},                 "variant",       { "FIELD", "INFO",   "FORMAT", "BOTH" } }, \
    [DT_SAM]   = { "SAM",       false, true,  true,   0b100,  DT_SAM,    DT_BAM,   sam_vb_size,    sam_vb_zip_dl_size,   HDR_OK_0,   SAM_CONTIG_FMT, '@', NULL,                   NULL,               sam_header_inspect,     is_sam,       sam_zip_initialize,     sam_zip_after_segconf,   sam_zip_after_vbs, sam_zip_finalize,    sam_zip_end_of_z,      sam_zip_init_vb,   sam_zip_after_compute,     sam_zip_set_vb_header_specific,   NULL,                            sam_zip_modify,   sam_seg_initialize,     sam_seg_txt_line,     NULL,                sam_seg_is_big,       sam_seg_is_small,     sam_seg_finalize,     sam_segconf_finalize,   sam_deep_zip_merge,  false,       sam_zip_comp_cb,   sam_zip_after_compress,   sam_stats_reallocate,   sam_zip_genozip_header,   sam_piz_genozip_header,   sam_piz_load_sags,      sam_piz_dispatch_one_load_sag_vb,       sam_piz_header_init,   sam_piz_initialize,     sam_piz_finalize,    sam_piz_after_vb_header,   sam_piz_init_vb,     sam_piz_vb_recon_init,      sam_reset_line,     sam_piz_after_recon,        sam_piz_process_recon,    sam_piz_after_preproc_vb, sam_piz_preproc_finalize, sam_piz_xtra_line_data, sam_piz_is_skip_section,    sam_reconstruct_SEQ_vs_ref,sam_piz_filter,        sam_piz_container_cb,     sam_piz_con_item_cb, NUM_SAM_SPECIAL,     SAM_SPECIAL,     NUM_SAM_TRANS,   SAM_TRANSLATORS,    "alignment",     { "FIELD", "QNAME",  "OPTION"         } }, \
    [DT_BAM]   = { "BAM",       true,  false, true,   0b100,  DT_SAM,    DT_BAM,   sam_vb_size,    sam_vb_zip_dl_size,   HDR_MUST_0, SAM_CONTIG_FMT, -1,  bam_is_header_done,     bam_unconsumed,     sam_header_inspect,     is_bam,       sam_zip_initialize,     sam_zip_after_segconf,   sam_zip_after_vbs, sam_zip_finalize,    sam_zip_end_of_z,      sam_zip_init_vb,   sam_zip_after_compute,     sam_zip_set_vb_header_specific,   NULL,                            bam_zip_modify,   bam_seg_initialize,     bam_seg_txt_line,     bam_assseg_line,     sam_seg_is_big,       sam_seg_is_small,     sam_seg_finalize,     sam_segconf_finalize,   sam_deep_zip_merge,  true,        sam_zip_comp_cb,   sam_zip_after_compress,   sam_stats_reallocate,   sam_zip_genozip_header,   sam_piz_genozip_header,   sam_piz_load_sags,      sam_piz_dispatch_one_load_sag_vb,       sam_piz_header_init,   sam_piz_initialize,     sam_piz_finalize,    sam_piz_after_vb_header,   sam_piz_init_vb,     sam_piz_vb_recon_init,      sam_reset_line,     sam_piz_after_recon,        sam_piz_process_recon,    sam_piz_after_preproc_vb, sam_piz_preproc_finalize, sam_piz_xtra_line_data, NULL,                       NULL,                      sam_piz_filter,        sam_piz_container_cb,     0/*cb only in SAM*/, NUM_SAM_SPECIAL,     SAM_SPECIAL,     NUM_SAM_TRANS,   SAM_TRANSLATORS,    "alignment",     { "FIELD", "DESC",   "OPTION"         } }, \
    [DT_CRAM]  = { "CRAM",      true,  true,  true,   0b100,  DT_SAM,    DT_CRAM,  sam_vb_size,    sam_vb_zip_dl_size,   HDR_MUST_0, SAM_CONTIG_FMT, -1,  bam_is_header_done,     bam_unconsumed,     sam_header_inspect,     is_cram,      sam_zip_initialize,     sam_zip_after_segconf,   sam_zip_after_vbs, sam_zip_finalize,    sam_zip_end_of_z,      sam_zip_init_vb,   sam_zip_after_compute,     sam_zip_set_vb_header_specific,   NULL,                            NULL,             bam_seg_initialize,     bam_seg_txt_line,     bam_assseg_line,     sam_seg_is_big,       sam_seg_is_small,     sam_seg_finalize,     sam_segconf_finalize,   sam_deep_zip_merge,  true,        sam_zip_comp_cb,   sam_zip_after_compress,   sam_stats_reallocate,   sam_zip_genozip_header,   sam_piz_genozip_header,   sam_piz_load_sags,      sam_piz_dispatch_one_load_sag_vb,       sam_piz_header_init,   sam_piz_initialize,     sam_piz_finalize,    sam_piz_after_vb_header,   sam_piz_init_vb,     sam_piz_vb_recon_init,      sam_reset_line,     sam_piz_after_recon,        sam_piz_process_recon,    sam_piz_after_preproc_vb, sam_piz_preproc_finalize, sam_piz_xtra_line_data, NULL,                       NULL,                      sam_piz_filter,        sam_piz_container_cb,     0/*cb only in SAM*/, NUM_SAM_SPECIAL,     SAM_SPECIAL,     NUM_SAM_TRANS,   SAM_TRANSLATORS,    "alignment",     { "FIELD", "DESC",   "OPTION"         } }, \
    [DT_FASTQ] = { "FASTQ",     false, true,  true,   0,      DT_FASTQ,  DT_NONE,  fastq_vb_size,  fastq_vb_zip_dl_size, HDR_NONE,   NULL,           -1,  NULL,                   fastq_unconsumed,   NULL,                   is_fastq,     fastq_zip_initialize,   fastq_zip_after_segconf, NULL,              fastq_zip_finalize,  NULL,                  fastq_zip_init_vb, fastq_zip_after_compute,   fastq_zip_set_vb_header_specific, fastq_zip_set_txt_header_flags,  fastq_zip_modify, fastq_seg_initialize,   fastq_seg_txt_line,   fastq_assseg_line,   fastq_seg_is_big,     fastq_seg_is_small,   fastq_seg_finalize,   fastq_segconf_finalize, NULL,                false,       fastq_zip_comp_cb, fastq_zip_after_compress, NULL,                   fastq_zip_genozip_header, fastq_piz_genozip_header, NULL,                   NULL,                                   fastq_piz_header_init, fastq_piz_initialize,   NULL,                fastq_piz_after_vb_header, fastq_piz_init_vb,   NULL,                       fastq_reset_line,   NULL,                       fastq_piz_process_recon,  NULL,                     NULL,                     NULL,                   fastq_piz_is_skip_section,  fastq_recon_aligned_SEQ,   fastq_piz_filter,      fastq_piz_container_cb,   0,                   NUM_FASTQ_SPECIAL,   FASTQ_SPECIAL,   0,               {},                 "read",          { "FIELD", "QNAME",  "AUX"            } }, \
    [DT_FASTA] = { "FASTA",     false, false, false,  0,      DT_FASTA,  DT_NONE,  fasta_vb_size,  fasta_vb_zip_dl_size, HDR_NONE,   NULL,           -1,  NULL,                   fasta_unconsumed,   NULL,                   is_fasta,     fasta_zip_initialize,   NULL,                    NULL,              NULL,                NULL,                  NULL,              fasta_zip_after_compute,   fasta_zip_set_vb_header_specific, NULL,                            NULL,             fasta_seg_initialize,   fasta_seg_txt_line,   NULL,                fasta_seg_is_big,     fasta_seg_is_small,   fasta_seg_finalize,   fasta_segconf_finalize, NULL,                false,       NULL,              NULL,                     NULL,                   NULL,                     NULL,                     NULL,                   NULL,                                   NULL,                  fasta_piz_initialize,   NULL,                NULL,                      fasta_piz_init_vb,   NULL,                       NULL,               NULL,                       NULL,                     NULL,                     NULL,                     NULL,                   fasta_piz_is_skip_section,  NULL,                      fasta_piz_filter,      NULL,                     0,                   NUM_FASTA_SPECIAL,   FASTA_SPECIAL,   0,               {},                 "line",          { "FIELD", "DESC",   "ERROR!"         } }, \
    [DT_REF]   = { "REFERENCE", false, false, false,  0,      DT_REF,    DT_NONE,  fasta_vb_size,  fasta_vb_zip_dl_size, HDR_NONE,   NULL,           -1,  NULL,                   fasta_unconsumed,   NULL,                   is_ref,       ref_make_ref_init,      NULL,                    NULL,              ref_make_finalize,   NULL,                  NULL,              ref_make_after_compute,    NULL,                             NULL,                            NULL,             ref_make_seg_initialize,fasta_seg_txt_line,   NULL,                NULL,                 fasta_seg_is_small,   fasta_seg_finalize,   fasta_segconf_finalize, NULL,                false,       NULL,              ref_make_create_range,    NULL,                   ref_make_genozip_header,  NULL,                     NULL,                   NULL,                                   NULL,                  NULL,                   NULL,                NULL,                      NULL,                NULL,                       NULL,               NULL,                       NULL,                     NULL,                     NULL,                     NULL,                   NULL,                       NULL,                      NULL,                  0,                        0,                   0,                   {},              0,               {},                 "line",          { "FIELD", "DESC",   "ERROR!"         } }, \
    [DT_GFF]   = { "GFF",       false, true,  false,  0,      DT_GFF,    DT_NONE,  gff_vb_size,    0,                    HDR_OK,     NULL,           '#', NULL,                   gff_unconsumed,     gff_header_inspect,     is_gff,       gff_zip_initialize,     NULL,                    NULL,              NULL,                NULL,                  NULL,              NULL,                      NULL,                             NULL,                            NULL,             gff_seg_initialize,     gff_seg_txt_line,     NULL,                gff_seg_is_big,       gff_seg_is_small,     gff_seg_finalize,     gff_segconf_finalize,   NULL,                false,       NULL,              NULL,                     NULL,                   NULL,                     NULL,                     NULL,                   NULL,                                   NULL,                  NULL,                   NULL,                NULL,                      gff_piz_init_vb,     NULL,                       gff_reset_line,     NULL,                       NULL,                     NULL,                     NULL,                     NULL,                   NULL,                       NULL,                      gff_piz_filter,        gff_piz_container_cb,     0,                   NUM_GFF_SPECIAL,     GFF_SPECIAL,     0,               {},                 "sequence",      { "FIELD", "ATTRS",  "ENST"           } }, \
    [DT_ME23]  = { "23ANDME",   false, true,  false,  0,      DT_ME23,   DT_NONE,  0,              0,                    HDR_MUST,   NULL,           '#', NULL,                   NULL,               me23_header_inspect,    is_me23,      NULL,                   NULL,                    NULL,              NULL,                NULL,                  NULL,              NULL,                      NULL,                             NULL,                            NULL,             me23_seg_initialize,    me23_seg_txt_line,    NULL,                NULL,                 me23_seg_is_small,    me23_seg_finalize,    NULL,                   NULL,                false,       NULL,              NULL,                     NULL,                   NULL,                     NULL,                     NULL,                   NULL,                                   NULL,                  NULL,                   NULL,                NULL,                      NULL,                NULL,                       NULL,               NULL,                       NULL,                     NULL,                     NULL,                     NULL,                   NULL,                       NULL,                      NULL,                  NULL,                     0,                   0,                   {},              NUM_ME23_TRANS,  ME23_TRANSLATORS,   "SNP",           { "FIELD", "ERROR!", "ERROR!"         } }, \
    [DT_GNRIC] = { "GENERIC",   true,  false, false,  0,      DT_NONE,   DT_GNRIC, 0,              0,                    HDR_OK,     NULL,           -1,  generic_is_header_done, generic_unconsumed, NULL,                   NULL,         NULL,                   NULL,                    NULL,              NULL,                NULL,                  NULL,              NULL,                      NULL,                             NULL,                            NULL,             generic_seg_initialize, generic_seg_txt_line, generic_assseg_line, NULL,                 generic_seg_is_small, generic_seg_finalize, NULL,                   NULL,                false,       NULL,              NULL,                     NULL,                   NULL,                     NULL,                     NULL,                   NULL,                                   NULL,                  NULL,                   NULL,                NULL,                      NULL,                NULL,                       NULL,               NULL,                       NULL,                     NULL,                     NULL,                     NULL,                   NULL,                       NULL,                      NULL,                  NULL,                     0,                   NUM_GENERIC_SPECIAL, GENERIC_SPECIAL, 0,               {},                 "N/A",           { "FIELD", "ERROR!", "ERROR!"         } }, \
    [DT_LOCS]  = { "LOCS",      true,  false, false,  0,      DT_NONE,   DT_LOCS,  0,              0,                    HDR_MUST,   NULL,           -1,  locs_is_header_done,    locs_unconsumed,    NULL,                   NULL,         NULL,                   NULL,                    NULL,              NULL,                NULL,                  NULL,              NULL,                      NULL,                             NULL,                            NULL,             locs_seg_initialize,    locs_seg_txt_line,    NULL,                NULL,                 locs_seg_is_small,    locs_seg_finalize,    NULL,                   NULL,                false,       NULL,              NULL,                     NULL,                   NULL,                     NULL,                     NULL,                   NULL,                                   NULL,                  NULL,                   NULL,                NULL,                      NULL,                NULL,                       NULL,               NULL,                       NULL,                     NULL,                     NULL,                     NULL,                   NULL,                       NULL,                      NULL,                  NULL,                     0,                   NUM_LOCS_SPECIAL,   LOCS_SPECIAL,     NUM_LOCS_TRANS,  LOCS_TRANSLATORS,   "cluster" ,      { "FIELD", "ERROR!", "ERROR!"         } }, \
    [DT_BED]   = { "BED",       false, true,  false,  0,      DT_BED,    DT_NONE,  0,              0,                    HDR_OK,     NULL,           -1,  bed_is_header_done,     NULL,               NULL,                   is_bed,       bed_zip_initialize,     NULL,                    NULL,              NULL,                NULL,                  NULL,              NULL,                      NULL,                             NULL,                            NULL,             bed_seg_initialize,     bed_seg_txt_line,     NULL,                NULL,                 bed_seg_is_small,     bed_seg_finalize,     bed_segconf_finalize,   NULL,                false,       NULL,              NULL,                     NULL,                   NULL,                     NULL,                     NULL,                   NULL,                                   NULL,                  NULL,                   NULL,                NULL,                      NULL,                NULL,                       NULL,               NULL,                       NULL,                     NULL,                     NULL,                     NULL,                   NULL,                       NULL,                      NULL,                  NULL,                     0,                   NUM_BED_SPECIAL,    BED_SPECIAL,      0,               {},                 "line",          { "FIELD", "ERROR!", "ERROR!"         } }, \
}  
#define DATA_TYPE_FUNCTIONS_DEFAULT /* only applicable to (some) functions */ \
                 { "DEFAULT",   false, false, false,  0,      DT_NONE,   DT_NONE,  def_vb_size,    0,                    0,          0,              0,   def_is_header_done,     def_unconsumed,     0,                      NULL,         0,                      NULL,                    0,                 0,                   0,                     0,                 0,                         NULL,                             NULL,                            NULL,             0,                      0,                    textual_assseg_line, NULL,                 0,                    0,                    NULL,                   NULL,                false,       NULL,              NULL,                     NULL,                   NULL,                     NULL,                     NULL,                   NULL,                                   0,                     0,                      NULL,                0,                         0,                   0,                          NULL,               0,                          NULL,                     NULL,                     NULL,                     NULL,                   0,                          0,                         default_piz_filter,    0,                        0,                   0,                  {},             0,               {},                 "",              { "FIELD", "DTYPE1", "DTYPE2"         } }
...
#define DTPO(prop) (dt_props[flag.out_dt].prop)
...
/*                 num_fields        pos              chrom           qname            eol             toplevel             predefined        */ \
    [DT_VCF]   = { NUM_VCF_FIELDS,   VCF_POS,         CHROM,          DID_NONE,        {_VCF_EOL},     {_VCF_TOPLEVEL},     VCF_PREDEFINED    }, \
    [DT_BCF]   = { NUM_VCF_FIELDS,   VCF_POS,         CHROM,          DID_NONE,        {_VCF_EOL},     {_VCF_TOPLEVEL},     VCF_PREDEFINED    }, \
    [DT_SAM]   = { NUM_SAM_FIELDS,   SAM_POS,         CHROM,          SAM_QNAME,       {_SAM_EOL},     {_SAM_TOPLEVEL},     SAM_PREDEFINED    }, \
    [DT_BAM]   = { NUM_SAM_FIELDS,   SAM_POS,         CHROM,          SAM_QNAME,       {_SAM_EOL},     {_SAM_TOP2BAM},      SAM_PREDEFINED    }, \
    [DT_CRAM]  = { NUM_SAM_FIELDS,   SAM_POS,         CHROM,          SAM_QNAME,       {_SAM_EOL},     {_SAM_TOP2BAM},      SAM_PREDEFINED    }, \
    [DT_FASTQ] = { NUM_FASTQ_FIELDS, DID_NONE,        CHROM,          FASTQ_QNAME,     {_FASTQ_E1L},   {_FASTQ_TOPLEVEL},   FASTQ_PREDEFINED  }, \
    [DT_FASTA] = { NUM_FASTA_FIELDS, DID_NONE,        CHROM,          DID_NONE,        {_FASTA_EOL},   {_FASTA_TOPLEVEL},   FASTA_PREDEFINED  }, \
    [DT_REF]   = { NUM_REF_FIELDS,   DID_NONE,        CHROM,          DID_NONE,        {},             {},                  REF_PREDEFINED    }, \
    [DT_GFF]   = { NUM_GFF_FIELDS,   GFF_START,       CHROM,          DID_NONE,        {_GFF_EOL},     {_GFF_TOPLEVEL},     GFF_PREDEFINED    }, \
    [DT_ME23]  = { NUM_ME23_FIELDS,  ME23_POS,        CHROM,          DID_NONE,        {_ME23_EOL},    {_ME23_TOPLEVEL},    ME23_PREDEFINED   }, \
    [DT_GNRIC] = { NUM_GNRIC_FIELDS, DID_NONE,        DID_NONE,       DID_NONE,        {},             {_GNRIC_TOPLEVEL},   GNRIC_PREDEFINED  }, \
    [DT_LOCS]  = { NUM_LOCS_FIELDS,  DID_NONE,        DID_NONE,       DID_NONE,        {},             {_LOCS_TOPLEVEL},    LOCS_PREDEFINED   }, \
    [DT_BED]   = { NUM_BED_FIELDS,   BED_START,       CHROM,          DID_NONE,        {_BED_EOL},     {_BED_TOPLEVEL},     BED_PREDEFINED    }, \
}
...
#define DTFO(prop) (dt_fields[flag.out_dt        ].prop)
...
#define TRANSLATIONS { /* note: translation i is not significant. translations can be added or removed. */ \
    ...
    /* SAM to CRAM          */ { DT_SAM,     false,   DT_CRAM,   { _SAM_TOP2BAM },       1,   sam_header_sam2bam,  true,     false, NULL                  }, /* BAM is expected to be smaller, edge cases will be handled by buf_alloc in sam_reset_line */ \
    ...
    /* Binary SAM to CRAM   */ { DT_SAM,     true,    DT_CRAM,   { _SAM_TOP2BAM },       1,   NULL,                true,     true,  NULL                  }, /* BAM to BAM */ \
...
extern DataType dt_get_dt_for_genozip_header (DataType dt, Codec src_codec);
extern rom dt_name (DataType data_type);
extern rom dt_name_faf (void);
