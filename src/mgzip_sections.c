...
FlagsMgzip mgzip_piz_calculate_mgzip_flags (CompIType comp_i, Codec src_codec)
{
    ...
    if (!mgzip_section_was_read && (flag.only_headers == SEC_GZ_ISIZES+1 || flag.only_headers == SHOW_ALL_HEADERS))
        bgzf_load_isizes (comp_i, C(NONE), true); 

    if (!mgzip_section_was_read && (flag.only_headers == SEC_GZ_DIGESTS+1 || flag.only_headers == SHOW_ALL_HEADERS))
