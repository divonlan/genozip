...
static Buffer qname_dict    = {};
static Buffer qname_nodes   = {}; // array of QnameNode
static Buffer qname_hashtab = {}; // array of uint32_t - index into qname_nodes
...
void kraken_seg_initialize (VBlockP vb)
{
    ...
    qname_seg_initialize (VB, QNAME1, KRAKEN_QNAME); 
}
...
static void kraken_seg_kmers (VBlockP vb, rom value, int32_t value_len, rom taxid, unsigned taxid_len) // must be signed
{
    ...
    for (unsigned k=0; k < num_kmers; k++) {
        ...
        if (str_issame_(STRa(taxid), STRi(item,0)))
...
CONTAINER_CALLBACK (kraken_piz_container_cb)
{
    ...
    // when pizzing a kraken to be filtered by itself (loaded or stored) (useful only for testing) - apply :kraken_is_included_loaded" filter 
             ...
             (   ( kraken_is_loaded && !kraken_is_included_loaded (vb, STRlst (KRAKEN_QNAME)))
...
bool kraken_piz_initialize (CompIType comp_i __attribute__((unused)))
{
    ...
    ASSERT (z_file->num_txt_files <= 2, "--kraken requires a .kraken.genozip file with up to 2 components, but %s has %u components",
            z_name, z_file->num_txt_files);
...
void kraken_load (void)
{
    ...
    if (!filename_has_ext (flag.reading_kraken, ".genozip")) // note: we don't test FileType, to allow kraken flexibility on file name as there is no standard extension
        kraken_get_genozip_file();

    z_file = file_open_z_read (flag.reading_kraken);    
    zfile_read_genozip_header (0, HARD_FAIL);
    
    ASSINP (Z_DT(KRAKEN), "expected %s to be a genozip'ed kraken output file, but its a %s file. Tip: compress the output file generated by kraken2 with \"genozip --input kraken\"", 
            z_name, z_dt_name());

    z_file->basename = filename_base (flag.reading_kraken, false, "(kraken-file)", NULL, 0);

    Dispatcher dispachter = piz_z_file_initialize();
    bool kraken_loaded = piz_one_txt_file (dispachter, false, false, COMP_NONE, COMP_NONE, false);

    kraken_filename = filename_make_unix (z_name); // full-path unix-style filename, allocates memory
