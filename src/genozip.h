...
// unaligned access using __attribute__((aligned(1))) with this attribute is slower than
// unaligned access without it: while violating C spec, unaligned access supported on modern x86 and ARM (but not on older ones)
// the Makefile may shut it off with -DUNALIGNED=
#ifndef UNALIGNED
#define UNALIGNED __attribute__((aligned(1)))
#endif

typedef uint16_t UNALIGNED unaligned_uint16_t;
typedef uint32_t UNALIGNED unaligned_uint32_t;
typedef uint64_t UNALIGNED unaligned_uint64_t;
typedef float    UNALIGNED unaligned_float;
...
#define GENOZIP_LICENSE   "GENOZIP_LICENSE"   // similar to --licfile
...
#define MAX_FIELDS 2047  // Maximum number of fields in a line (eg VCF variant, SAM alignment etc), including VCF/FORMAT fields, VCF/INFO fields GVF/ATTR fields, SAM/AUX fields etc. Also the max n_items in a container.
...
// ------------------------------------------------------------------------------------------------------------------------
typedef struct VBlock *restrict VBlockP;
typedef const struct VBlock *restrict ConstVBlockP;
#define NULL_VB ((VBlockP)0)
typedef struct VBlockPool *VBlockPoolP;
typedef struct File *restrict FileP;
typedef const struct File *restrict ConstFileP;
typedef struct Buffer *BufferP;
typedef const struct Buffer *ConstBufferP;
typedef struct Buffer *restrict Buffer𐤐;
...
typedef struct Buffer *BitsP;
typedef const struct Buffer *ConstBitsP;
typedef struct Buffer *restrict Bits𐤐;
typedef const struct Buffer *restrict ConstBits𐤐;
...
// union of pointers to all sizes of containers. designed to avoid strict-aliasing issues. add as needed.
typedef union {
    // dereferencing is only through h, never through the other fields, but other fields (pointers) might be assigned
    struct Container_0 *restrict h; 
    #define F(n) const struct Container_##n *c##n
    F(1);  F(2);  F(3);  F(4);  F(5);  F(6); F(7); F(8); F(9); F(10); 
    F(11); F(12); F(13); F(14); F(15); F(16); 
    F(MAX_FIELDS); F(MAX_QNAME_ITEMS); F(MAX_TOKENS); F(MAX_FORMAT_FIELDS); F(FASTQ_NUM_TOP_LEVEL_FIELDS);
    F(NUM_SA_ITEMS) ; F(MAX_CB_ITEMS) ; F(MAX_CSQ_ITEMS); F(MAX_ALLELES); F(DOUBLE_MAX_ALLELES) ;
    #undef F
} ContainerP __attribute__((__transparent_union__));

// likewise, union of pointers to all sizes of multiplexors
typedef union {
    struct Multiplexer1000 *m; // dereference only through this (larger than the largest)
    #define M(n) const struct Multiplexer##n *m##n
    M(2); M(3); M(4); M(5); M(6); M(7); M(10);
    M(N_DOSAGES_IN_MUX); M(N_DOSAGEx7_IN_MUX); M(N_DOSAGEx51_IN_MUX); M(MAX_DP_FOR_MUX);
    #undef M
} MultiplexerP __attribute__((__transparent_union__));
...
typedef union DictId {
    ...
    uint16_t map_key[4];      // [0] is the main key into vb/z_file->d2d_map (other 3 are unused)
    struct {
        #define MAP_KEY(d) (__builtin_bswap16((d).map_key[0])) // note: swap so 2nd dict_id character is LSB - so  sub-contexts created with sub_dict_id (which differ by the 2nd char) appear near each other in d2d_map - usually in same L1 cache line (to do: don't swap on big endian (bug 1117)
...
typedef PosType64 *restrict PosType64𐤐;
...
typedef const char *restrict rom𐤐;      
typedef const uint8_t *bytes;   // read-only array of bytes
typedef const uint8_t *restrict bytes𐤐;   
...
typedef packed_enum { // 1 byte
    ...
    // gzip codecs
    CODEC_GZ=2, CODEC_BGZF=20, CODEC_GZBL=40, // General GZIP codecs 
    CODEC_IL1M=34, CODEC_IL4M=39, // Illumina GZIP codecs 
    ...
    CODEC_TMPL    = 41, // compress Element QUAL data
...
typedef packed_enum { // 1 byte
    ...
    SEC_GZ_ISIZES       = 15, // Per-component section (optional): contains the uncompressed sizes of the source file mgzip block (called SEC_BGZF until 15.0.62 and SEC_MGZIP until 15.0.79)
    ...
    SEC_GZ_DIGESTS      = 23, // Per-component section (optional): 32b digests of each gz block in an exactable file: introduced 15.0.80
    NUM_SEC_TYPES 
} SectionType;
...
typedef packed_enum { NO_COMMAND=0, ZIP='z', PIZ='d' /* this is unzip */, LIST='l', LICENSE='L', VERSION='V', HELP=140, SHOW_HEADERS=10, BIOPSY_BYTES=148, SHOW_BAI=11, GENERATE_IL1M=153, SHOW_FLAVOR=156, SHOW_GZ=157, DUMP_GZ_BLOCK=158, RM_CACHE=253, TEST_AFTER_ZIP=254 } CommandType; 
extern CommandType primary_command;
#define IS_ZIP           (flag.command == ZIP)
#define IS_PIZ           (flag.command == PIZ)
#define IS_LIST          (flag.command == LIST)
#define IS_SHOW_HEADERS  (flag.command == SHOW_HEADERS)
#define IS_SHOW_BAI      (flag.command == SHOW_BAI)
#define IS_SHOW_GZ       (flag.command == SHOW_GZ)
#define IS_DUMP_GZ_BLOCK (flag.command == DUMP_GZ_BLOCK)
...
#define STRw𐤐(x) char *restrict x; uint32_t x##_len    // writeable
...
#define emSTRl(name,multi,len) extern char name##s[multi][len]; extern uint32_t name##_len##s[multi]
...
#define STRlast(name,did_i)    ASSERT_LAST_TXT_VALID(CTX(did_i)); rom name UNUSED = last_txt((VBlockP)(vb), did_i); uint32_t name##_len UNUSED = CTX(did_i)->last_txt.len
...
#define STR_ARRAY(x,n) STR_ARRAY_(x,n); uint32_t n_##x##s UNUSED
...
#define STR𐤐(x)  rom𐤐 x,      uint32_t x##_len    
#define STR8p(x) bytes x,     uint32_t x##_len    
#define STR8𐤐(x) bytes𐤐 x,    uint32_t x##_len    
#define STRc𐤐(x)  char * restrict x, uint32_t x##_len  // string is fixed-length, but editable 
#define STRc(x)  char *x,     uint32_t x##_len  // string is fixed-length, but editable 
#define STR8c(x) uint8_t *x,  uint32_t x##_len
#define STR8c𐤐(x) uint8_t *restrict x, uint32_t x##_len
#define pSTRp(x) rom *x,      uint32_t *x##_len  
#define 𐤐STR𐤐(x) rom𐤐 *restrict x, uint32_t *restrict x##_len  
#define qSTRp(x) char *x,     uint32_t *x##_len // function populates a string and updates its length
#define qSTR𐤐(x) char *restrict x, uint32_t *restrict x##_len // function populates a string and updates its length
#define qSTR8p(x) uint8_t *x, uint32_t *x##_len 
#define STR𐤐s(x) uint32_t n_##x##s, rom𐤐 restrict *x##s, const uint32_t *restrict x##_lens  
...
#define STRb8(buf)  (uint8_t*)(buf).data, (buf).len                  
...
#define QTYPE_LOWEST_VALUE -5
typedef enum { QSAM2  = -5, // SAM QNAME2, while segging deep (for example, consensus reads flavor)
               QSAM   = -4, // SAM qname, while segging deep
               Q1or3  = -3, // used in "only_q"
               QANY   = -2, 
               QNONE  = -1,
               ...
               QEMBED = 3,  // SAM/BAM/FASTQ: Embedded QNAME within another QNAME
               NUM_QTYPES } QType; // NOTE: NUM_QTYPES is in SectionHeaderTxtHeader 
#define QTYPE_NAME { "QSAM2"/*QTYPE_LOWEST_VALUE*/, "QSAM", "Q1or3", "QANY", "QNONE", "QNAME"/*0*/, "QNAME2", "LINE3", "EMBED" }
#define QTYPE_DID  {  FASTQ_QNAME,  FASTQ_QNAME2,  FASTQ_LINE3,  SAM_QEMBED }
#define QTYPE_DNUM { _FASTQ_QNAME, _FASTQ_QNAME2, _FASTQ_LINE3, _SAM_QEMBED }
...
#define CONTAINER_FILTER_FUNC(func) bool func(VBlockP vb, DictId dict_id, ContainerP con, unsigned rep, int item, ReconType *reconstruct)

// called after reconstruction of each repeat, IF Container.callback or Container.is_top_level is set
#define CONTAINER_CALLBACK(func) void func(VBlockP vb_, DictId dict_id, bool is_top_level, unsigned rep, ContainerP con, \
...
typedef packed_enum { BGZF_NOT_INITIALIZED    = -100,  
                      BGZF_EXACT              = -1,   // PIZ: use BGZF compression recorded in .genozip file 
                      BGZF_EXACT_STRICT       = -2,   // PIZ: user requested --bgzf=exact-strict
                      BGZF_EXACT_FAILED       = -3,   // PIZ: user requested --bgzf=exact[-strict], but it is not possible 
...
#define IS_PIZ_EXACT (flag.bgzf == BGZF_EXACT || flag.bgzf == BGZF_EXACT_STRICT)
...
extern noreturn void error_exit (bool show_stack, bool is_error);
...
#define WARN_IF(condition, format, ...)      ({ if (__builtin_expect ((condition), 0) && !flag.explicit_quiet) warn ((format), ##__VA_ARGS__); })
#define WARN_ONCE(format, ...)               ({ static bool warning_shown = false; if (!flag.quiet && !test_and_set_relaxed (warning_shown)) warn ((format), ##__VA_ARGS__); }) 

#define ASSERTW(condition, format, ...)      ({ if (__builtin_expect (!(condition), 0) && !flag.quiet) warn ((format), ##__VA_ARGS__); })
#define ASSERTWD(condition, format, ...)     ({ if (__builtin_expect (!(condition), 0) && flag.debug && !flag.quiet) warn ((format), ##__VA_ARGS__); })

#define ASSWRET(condition, ret, format, ...) ({ if (__builtin_expect (!(condition), 0)) { if (!flag.quiet) { warn ((format), ##__VA_ARGS__); } return ret; } })

// Critical error - restart with a flag that will solve the issue
#define RESTART(add_cmd_option, format, ...) ({ error_restart (add_cmd_option, (format), ##__VA_ARGS__); })
