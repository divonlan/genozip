...
static int bsc_detect_segments_serial(void *vb, BscSegmentationModel * RESTRICT model, const unsigned char * RESTRICT input, int n)
{
...
static int bsc_detect_segments_recursive(void *vb, BscSegmentationModel * model0, BscSegmentationModel * model1, const unsigned char * input, int n, int * segments, int k, int features)
{
    ...
    int blockSize = bsc_detect_segments_serial(vb, model0, input, n);
    ...
    int leftResult = bsc_detect_segments_recursive(vb, model0, model1, input, blockSize, segments, k - 1, features);
    if (leftResult < LIBBSC_NO_ERROR) return leftResult;

    int rightResult = bsc_detect_segments_recursive(vb, model0, model1, input + blockSize, n - blockSize, segments + leftResult, k - leftResult, features);
    if (rightResult < LIBBSC_NO_ERROR) return rightResult;

    return leftResult + rightResult;
}

int bsc_detect_segments(void *vb, const unsigned char * input, int n, int * segments, int k, int features)
{
    if (n < DETECTORS_BLOCK_SIZE || k == 1)
    {
        segments[0] = n;
    ...
    if (BscSegmentationModel * model0 = (BscSegmentationModel *)bsc_malloc (vb, sizeof(BscSegmentationModel)))
    {
        if (BscSegmentationModel * model1 = (BscSegmentationModel *)bsc_malloc (vb, sizeof(BscSegmentationModel)))
        {
            int result = bsc_detect_segments_recursive(vb, model0, model1, input, n, segments, k, features);

            bsc_free (vb, model1); bsc_free (vb, model0);
...
int bsc_detect_contextsorder(void *vb, const unsigned char * RESTRICT input, int n, int features)
{
    ...
    {
        if (unsigned char * buffer = (unsigned char *)bsc_malloc (vb, DETECTORS_NUM_BLOCKS * DETECTORS_BLOCK_SIZE * sizeof(unsigned char)))
        {
            ...
            sortingContexts = bsc_detect_contextsorder (vb, buffer, DETECTORS_NUM_BLOCKS * DETECTORS_BLOCK_SIZE, features);
    ...
    if (unsigned char * RESTRICT buffer = (unsigned char *)bsc_malloc (vb, n * sizeof(unsigned char)))
    {
        if (int * RESTRICT bucket0 = (int *)bsc_zero_malloc (vb, ALPHABET_SIZE * ALPHABET_SIZE * sizeof(int)))
        {
            if (int * RESTRICT bucket1 = (int *)bsc_zero_malloc (vb, ALPHABET_SIZE * ALPHABET_SIZE * sizeof(int)))
            {
...
static long long bsc_estimate_reordering(BscReorderingModel * model, int recordSize)
{
    long long entropy = 0;
...
int bsc_detect_recordsize(void *vb, const unsigned char * RESTRICT input, int n, int features)
{
    ...
    if ((n > DETECTORS_NUM_BLOCKS * DETECTORS_BLOCK_SIZE) && (features & LIBBSC_FEATURE_FASTMODE))
    {
        if (unsigned char * buffer = (unsigned char *)bsc_malloc (vb, DETECTORS_NUM_BLOCKS * DETECTORS_BLOCK_SIZE * sizeof(unsigned char)))
        {
            ...
            result = bsc_detect_recordsize(vb, buffer, DETECTORS_NUM_BLOCKS * DETECTORS_BLOCK_SIZE, features);
    ...
    if (BscReorderingModel * RESTRICT model = (BscReorderingModel *)bsc_malloc (vb, sizeof(BscReorderingModel)))
    {
