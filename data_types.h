...
typedef enum { VCF_CHROM, VCF_POS, VCF_ID, VCF_REFALT, VCF_QUAL, VCF_FILTER, VCF_INFO, VCF_FORMAT, VCF_SAMPLES, VCF_EOL, VCF_TOPLEVEL, 
               VCF_oCHROM, VCF_oPOS, VCF_oREFALT, VCF_oXSTRAND, VCF_COORDS, VCF_oSTATUS, VCF_COPY_POS, VCF_LIFT_REF, VCF_COPYSTAT, VCF_TOPLUFT, // Liftover data - must appear in same order in any data type that has it
...
/* num_fields        pos         test_regions    nonref        ochrom,     eol          toplevel names (including extend fields) - max 8 characters - 2 first chars must be unique within each data type (for dict_id_to_did_i_map) */ \
  ...
  {NUM_CHAIN_FIELDS, DID_I_NONE, CHAIN_NAMEDST,  DID_I_NONE,   DID_I_NONE, CHAIN_EOL,   CHAIN_TOPLEVEL,   { "NaMEDST", "SrANDDST", "StARTDST", "EnDDST", "SiZEDST", "NAMESRC", "SRANDSRC", "STARTSRC", "ENDSRC", "SIZESRC", "CHAIN", "SCORE", "ID", "SET", "SIZE", "GAPS", "EOL", TOPLEVEL } }, /* unique first 2 letters */ \
