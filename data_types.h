...
typedef struct DataTypeProperties {
    ...
    void (*piz_initialize)(void); // called at the beginning of each output txt_file (after the global sections have been read already)
    void (*piz_finalize)(void);   // called by main thread after each z_file reconstruction is done
    bool (*piz_read_one_vb)(VBlockP, ConstSecLiEntP); // called by main thread after all sections of a VB have been read, before dispatching the compute thread
...
/*    name         is_bin bin_type has_ra ht sizeof_vb      sizeof_zip_dataline   txt_headr header_contigs  1st  is_header_done       unconsumed        inspect_txt_header,     zip_initialize          zip_finalize        zip_read_one_vb        zip_after_compute          zip_dts_flag          seg_initialize          seg_txt_line          seg_is_small,         seg_finalize,         compress                  piz_header_init      piz_initialize          piz_finalize         piz_read_one_vb          is_skip_section            reconstruct_seq            container_filter       container_cb              num_special        special        num_trans        translators        release_vb           destroy_vb           cleanup_memory          show_sections_line stat_dict_types                 */ \
    ...
    { "KRAKEN",    false, DT_NONE, NO_RA, 1, 0,             0,                    HDR_NONE, NULL,           -1,  NULL,                NULL,             NULL,                   NULL,                   NULL,               NULL,                  kraken_zip_after_compute,  NULL,                 kraken_seg_initialize,  kraken_seg_txt_line,  kraken_seg_is_small,  kraken_seg_finalize,  NULL,                     NULL,                kraken_piz_initialize,  NULL,                NULL,                    NULL,                      NULL,                      NULL,                  kraken_piz_container_cb,  0,                 {},            0,               {},                NULL,                NULL,                NULL,                   "Sequences",       { "FIELD", "QNAME",  "ERROR!" } }, \
}  
...
/* num_fields        pos         test_regions    info        nonref        ochrom,     eol          toplevel names (including extend fields) - max 8 characters - 2 first chars must be unique within each data type (for dict_id_to_did_i_map) */ \
  ...
  {NUM_SAM_FIELDS,   SAM_POS,    SAM_POS,        -1,         SAM_NONREF,   -1,         SAM_EOL,     SAM_TOPLEVEL,     { "RNAME", "QNAME", "FLAG", "POS", "MAPQ", "CIGAR", "RNEXT", "PNEXT", "TLEN", "OPTIONAL", "SQBITMAP", "NONREF", "NONREF_X", "GPOS", "STRAND", "QUAL", "DOMQRUNS", "EOL", "BAM_BIN", TOPLEVEL, "TOP2BAM", "TOP2FQ", "E2:Z", "2NONREF", "N2ONREFX", "2GPOS", "S2TRAND", "U2:Z", "D2OMQRUN", "TAXID" } }, \
  {NUM_FASTQ_FIELDS, -1,         -1,             -1,         FASTQ_NONREF, -1,         FASTQ_E1L,   FASTQ_TOPLEVEL,   { "CONTIG", "DESC", "E1L", "SQBITMAP", "NONREF", "NONREF_X", "GPOS", "STRAND", "E2L", "QUAL", "DOMQRUNS", TOPLEVEL, "TAXID" } }, \
  {NUM_FASTA_FIELDS, -1,         FASTA_LINEMETA, -1,         FASTA_NONREF, -1,         FASTA_EOL,   FASTA_TOPLEVEL,   { "CONTIG", "LINEMETA", "EOL", "DESC", "COMMENT", "NONREF", "NONREF_X", TOPLEVEL, "TAXID" } }, \
  ...
  {NUM_SAM_FIELDS,   SAM_POS,    SAM_POS,        -1,         SAM_NONREF,   -1,         SAM_EOL,     SAM_TOP2BAM,      { "RNAME", "QNAME", "FLAG", "POS", "MAPQ", "CIGAR", "RNEXT", "PNEXT", "TLEN", "OPTIONAL", "SQBITMAP", "NONREF", "NONREF_X", "GPOS", "STRAND", "QUAL", "DOMQRUNS", "EOL", "BAM_BIN", TOPLEVEL, "TOP2BAM", "TOP2FQ", "E2:Z", "2NONREF", "N2ONREFX", "2GPOS", "S2TRAND", "U2:Z", "D2OMQRUN", "TAXID" } }, \
  {NUM_VCF_FIELDS,   VCF_POS,    VCF_POS,        VCF_INFO,   -1,           VCF_oCHROM, VCF_EOL,     VCF_TOPLEVEL,     { "CHROM", "POS", "ID", "REF+ALT", "QUAL", "FILTER", "INFO", "FORMAT", "SAMPLES", "EOL", TOPLEVEL, "oCHROM", "oPOS", "oREF", "oSTRAND", "oALTRULE", "o$TATUS" } }, \
  ...
  {NUM_KRAKEN_FIELDS,-1,         -1,             -1,         -1,           -1,         KRAKEN_EOL,  KRAKEN_TOPLEVEL,  { "CU", "QNAME", "TAXID", "SEQLEN", "KMERS", "KMERTAX", "KMERLEN", "EOL", TOPLEVEL, "TOP2HASH" } }, \
}
...
#define TRANSLATIONS { \
    ...
    /* genocat --taxid      */ { DT_KRAKEN,  false, DT_NONE,   KRAKEN_TOP2TAXID,  1,   NULL,                true,     false, &flag.kraken_taxid }, \
}
