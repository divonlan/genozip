...
typedef struct DataTypeProperties {
    ...
    const char *stat_dict_types[3]; // the dictionary type displayed in --stats
} DataTypeProperties;
...
/*    name         is_bin bin_type has_ra ht sizeof_vb      sizeof_zip_dataline   txt_headr header_contigs  1st  is_header_done       unconsumed        inspect_txt_header,     zip_initialize          zip_finalize        zip_read_one_vb        zip_after_compute          zip_dts_flag          seg_initialize          seg_txt_line          seg_is_small,         seg_finalize,         compress                  piz_header_init      piz_initialize          piz_finalize         piz_read_one_vb          is_skip_section             reconstruct_seq            container_filter       container_cb              num_special        special        num_trans        translators        release_vb           destroy_vb           cleanup_memory          show_sections_line stat_dict_types                 */ \
    ...
    { "GFF3",      false, DT_NONE, RA,    1, 0,             0,                    HDR_OK,   NULL,           '#', NULL,                NULL,             NULL,                   NULL,                   NULL,               NULL,                  NULL,                      NULL,                 gff3_seg_initialize,    gff3_seg_txt_line,    gff3_seg_is_small,    gff3_seg_finalize,    NULL,                     NULL,                NULL,                   NULL,                NULL,                    NULL,                       NULL,                      gff3_piz_filter,       NULL,                     0,                 {},            0,               {},                NULL,                NULL,                NULL,                   "Sequences",       { "FIELD", "ATTRS",  "ITEMS"  } }, \
    ...
    { "BCF",       false, DT_NONE, RA,    1, vcf_vb_size,   vcf_vb_zip_dl_size,   HDR_MUST, VCF_CONTIG_FMT, '#', NULL,                NULL,             vcf_inspect_txt_header, vcf_zip_initialize,     txtheader_finalize, vcf_zip_read_one_vb,   vcf_zip_after_compute,     NULL,                 vcf_seg_initialize,     vcf_seg_txt_line,     vcf_seg_is_small,     vcf_seg_finalize,     NULL,                     vcf_header_piz_init, NULL,                   txtheader_finalize,  vcf_piz_read_one_vb,     vcf_piz_is_skip_section,    NULL,                      vcf_piz_filter,        vcf_piz_container_cb,     NUM_VCF_SPECIAL,   VCF_SPECIAL,   NUM_VCF_TRANS,   VCF_TRANSLATORS,   vcf_vb_release_vb,   vcf_vb_destroy_vb,   vcf_vb_cleanup_memory,  "Variants",        { "FIELD", "INFO",   "FORMAT" } }, \
...
typedef enum { GFF3_SEQID, GFF3_SOURCE, GFF3_TYPE, GFF3_START, GFF3_END, GFF3_SCORE, GFF3_STRAND, GFF3_PHASE, GFF3_ATTRS, GFF3_EOL, GFF3_TOPLEVEL, GFF3_COMMENT, NUM_GFF3_FIELDS } Gff3Fields;
...
/* num_fields        pos         nonref        ochrom,     eol          toplevel names (including extend fields) - max 8 characters - 2 first chars must be unique within each data type (for dict_id_to_did_i_map) */ \
  ...
  {NUM_FASTQ_FIELDS, DID_I_NONE, FASTQ_NONREF, DID_I_NONE, FASTQ_E1L,   FASTQ_TOPLEVEL,   { "CONTIG", "DESC", "E1L", "SQBITMAP", "NONREF", "NONREF_X", "GPOS", "STRAND", "E2L", "QUAL", "DOMQRUNS", TOPLEVEL, "TAXID", "LINE3" } }, \
