...
static int32_t seg_one_field (VariantBlock *vb, const char *str, unsigned len, unsigned line_i, VcfFields f, 
                              bool *is_new) // optional out
{
    ...
    MtfNode *node;
    int32_t node_index = mtf_evaluate_snip (vb, ctx, str, len, false, &node, is_new);
...
static bool seg_chrom_field (VariantBlock *vb, const char *chrom_str, unsigned chrom_str_len, unsigned line_i)
{
    seg_one_field (vb, chrom_str, chrom_str_len, line_i, CHROM, NULL);

    if (! *vb->chrom) { // first line in this VB
        memcpy (vb->chrom, chrom_str, MIN (chrom_str_len, MAX_CHROM_LEN-1));
        vb->chrom[chrom_str_len] = 0;
        return true;
    }
 
    return !strncmp (chrom_str, vb->chrom, MIN (MAX_CHROM_LEN-1, chrom_str_len));
}

static void seg_pos_field (VariantBlock *vb, const char *pos_str, unsigned pos_len, unsigned line_i /* line in VCF file */)
{
    ...
    ASSERT (this_pos_64 >= 0 && this_pos_64 <= 0x7fffffff, "POS field to big in line %u, maximum allowed value is %u", line_i, 0x7fffffff);
    int32_t this_pos = (int32_t)this_pos_64;

    if (this_pos < vb->last_pos) 
        vb->is_sorted_by_pos = false;

    if (this_pos < vb->min_pos || vb->min_pos < 0)
        vb->min_pos = this_pos;

    if (this_pos > vb->max_pos)
        vb->max_pos = this_pos;
    ...
    seg_one_field (vb, pos_delta_str, len, line_i, POS, NULL);
                              ...
                              unsigned line_i) // line in original vcf file
{
    ...
    ASSERT (field_len, "Error: missing FORMAT field in line %u", line_i);
    ...
    if (field_len > 2 || (!dl->has_haplotype_data && field_len > 0)) {
        ...
        do {
            ASSERT (format_mapper.num_subfields < MAX_SUBFIELDS, 
                    "Error: FORMAT field in line %u has too many subfields, the maximum allowed is %u (excluding GT)", line_i, MAX_SUBFIELDS);

            DictIdType subfield = seg_get_format_subfield (&str, (unsigned *)&len, line_i);
    ...
    if (dl->has_genotype_data)
        buf_alloc (vb, &vb->line_gt_data, format_mapper.num_subfields * global_num_samples * sizeof(uint32_t), 1, "line_gt_data", line_i); 

    bool is_new;
    int32_t node_index = seg_one_field (vb, field_start, field_len, line_i, FORMAT, &is_new);
...
static void seg_info_field (VariantBlock *vb, DataLine *dl, const char *info_str, unsigned info_len, unsigned line_i)
{
    ...
    for (unsigned i=0; i < info_len + 1; i++) {
        ...
        if (reading_name) {
            ...
            ASSERT (iname_len <= MAX_INFO_NAMES_LEN, "Error: INFO field too long in line %u", line_i);

            if (c == '=') {  // end of name

                ASSERT (this_name_len > 0, "Error: INFO field in line %u, contains a = without a preceding subfield name", line_i);

                ASSERT (this_name[0] >= 64 && this_name[0] <= 127, 
                        "Error: INFO field in line %u, contains a name %.*s starting with an illegal character", line_i, this_name_len, this_name);
            ...
            if (c == ';') { // end of value
                ...
                ASSERT (this_value_len > 0, 
                        "Error: INFO field in line %u, subfield %.*s, does not contain a value", line_i, this_name_len, this_name);
                ...
                ((uint32_t *)mtf_i_buf->data)[mtf_i_buf->len++] = 
                    mtf_evaluate_snip (vb, iname_mapper.ctx[sf_i], this_value, this_value_len, false, &sf_node, NULL);
    ...
    MtfNode *node;
    bool is_new;
    uint32_t node_index = mtf_evaluate_snip (vb, info_ctx, iname, iname_len, false, &node, &is_new);
...
static void seg_increase_ploidy (VariantBlock *vb, unsigned new_ploidy, unsigned line_i, unsigned sample_i)
{
    unsigned vb_line_i = line_i - vb->first_line;
    ...
    for (unsigned i=0; i < vb_line_i; i++) {
        ...
        buf_alloc (vb, &dl->line, new_total_len, 1, "dl->line", line_i);
...
static void seg_haplotype_area (VariantBlock *vb, DataLine *dl, const char *str, unsigned len, unsigned line_i, unsigned sample_i,
                                bool is_vcf_string)
{
    // check ploidy
    unsigned ploidy=1;
    ...
    if (vb->ploidy && ploidy > vb->ploidy) 
        seg_increase_ploidy (vb, ploidy, line_i, sample_i);
    ...
    for (unsigned ht_i=0; ht_i < ploidy; ht_i++) {
        ...
        if (ploidy > 1 && ht_i < ploidy-1) {
            ...
            ASSERT (cell_phase_type != ' ', "Error: invalid VCF file - line %u - expecting a tab or newline after sample %u but seeing a space", line_i, sample_i+1);
                               ...
                               bool is_vcf_string)
{
    ...
    for (unsigned sf=0; sf < format_mapper->num_subfields; sf++) { // iterate on the order as in the line
        ...
        MtfNode *node;
        uint32_t node_index = mtf_evaluate_snip (vb, format_mapper->ctx[sf], cell_gt_data, len, false, &node, NULL);
        *(next++) = node_index;
...
static inline const char *seg_get_next_item (const char *str, unsigned *str_len, bool allow_newline, bool allow_tab, bool allow_colon, unsigned line_i, // line in vcf file,
                                             unsigned *len, char *separator, const char *item_name) // out
{
    ...
    ASSERT (*str_len, "Error: missing %s field on line_i=%u", item_name, line_i);

    ASSERT (str[i] != '\t' || strcmp (item_name, vcf_field_names[INFO]), 
           "Error: while segmenting %s on line_i=%u: expecting a NEWLINE after the INFO field, because this VCF file has no samples (individuals) declared in the header line",
            item_name, line_i);

    ABORT ("Error: while segmenting %s on line_i=%u: expecting a %s %s %s after \"%.*s\"", 
            item_name, line_i, 
                           ...
                           unsigned line_i) // line in original VCF file
{
    ...
    next_field = seg_get_next_item (field_start, &len, false, true, false, line_i, &field_len, &separator, "CHROM");
    seg_chrom_field (vb, field_start, field_len, line_i);

    // POS
    field_start = next_field;
    next_field = seg_get_next_item (field_start, &len, false, true, false, line_i, &field_len, &separator, "POS");
    seg_pos_field (vb, field_start, field_len, line_i);;

    // ID
    field_start = next_field;
    next_field = seg_get_next_item (field_start, &len, false, true, false, line_i, &field_len, &separator, "ID");
    seg_one_field (vb, field_start, field_len, line_i, ID, NULL);
    ...
    field_start = next_field;
    next_field = seg_get_next_item (field_start, &len, false, true, false, line_i, &field_len, &separator, "REF");

    unsigned alt_len;
    next_field = seg_get_next_item (next_field, &len, false, true, false, line_i, &alt_len, &separator, "ALT");
    seg_one_field (vb, field_start, field_len+alt_len+1, line_i, REFALT, NULL);

    // QUAL
    field_start = next_field;
    next_field = seg_get_next_item (field_start, &len, false, true, false, line_i, &field_len, &separator, "QUAL");
    seg_one_field (vb, field_start, field_len, line_i, QUAL, NULL);

    // FILTER
    field_start = next_field;
    next_field = seg_get_next_item (field_start, &len, false, true, false, line_i, &field_len, &separator, "FILTER");
    seg_one_field (vb, field_start, field_len, line_i, FILTER, NULL);

    // INFO
    field_start = next_field;
    next_field = seg_get_next_item (field_start, &len, global_num_samples==0, global_num_samples>0, false, line_i, &field_len, &separator, "INFO");
    seg_info_field (vb, dl, field_start, field_len, line_i);

    if (separator != '\n') {

        // FORMAT
        field_start = next_field;
        next_field = seg_get_next_item (field_start, &len, true, true, false, line_i, &field_len, &separator, "FORMAT");
        seg_format_field (vb, dl, field_start, field_len, line_i);

        ASSERT (separator == '\n' || dl->has_genotype_data || dl->has_haplotype_data, 
                "Error: expecting line_i=%u to end as it has no genotype or haplotype data, but it is not", line_i);
        ...
        while (separator != '\n') {
            ...
            if (dl->has_haplotype_data) { // FORMAT declares GT, we may have it or not
                ...
                field_start = next_field;
                next_field = seg_get_next_item (field_start, &len, true, true, dl->has_genotype_data, line_i, &field_len, &separator, "GT");
                seg_haplotype_area (vb, dl, field_start, field_len, line_i, sample_i, true);
            ...
            if (has_genotype_data) { // FORMAT declares other subfields, we may have them or not
                field_start = next_field;
                next_field = seg_get_next_item (field_start, &len, true, true, false, line_i, &field_len, &separator, "Non-GT");
                seg_genotype_area (vb, dl, field_start, field_len, line_i, true);
    ...
    if (sample_i < global_num_samples) 
        seg_add_samples_missing_in_line (vb, dl, &gt_line_len, sample_i, line_i);
