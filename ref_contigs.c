...
static void ref_contigs_show (const Buffer *contigs_buf, bool created)
{
    ...
    for (uint32_t i=0; i < cn_len; i++) {
        ...
        bool ext_ref = flag.reference == REF_EXTERNAL || flag.reference == REF_EXT_STORE || flag.reference == REF_LIFTOVER || flag.reference == REF_MAKE_CHAIN
                       || Z_DT(DT_REF);

        if (ext_ref && created)
            iprintf ("\"%s\" length=%"PRId64" i=%u gpos=%"PRId64" \"%s\"\n",
                     chrom_name,  cn[i].max_pos, cn[i].chrom_index, cn[i].gpos, cn[i].metadata.str);

        else if (ext_ref) 
            iprintf ("\"%s\" length=%"PRId64" i=%u gpos=%"PRId64" %s\n",
                     chrom_name,  cn[i].max_pos, cn[i].chrom_index, cn[i].gpos, display_acc_num (&cn[i].metadata.parsed.ac).s);
...
void ref_contigs_compress (Reference ref)
{
    ...
    buf_alloc (evb, &created_contigs, 0, MAX_(ZCTX(CHROM)->nodes.len, ref->ctgs.contigs.len), Contig, 1, "created_contigs");
        ...
        if (!last || r->chrom != last->chrom_index) {
            ...
            if (header_contigs) {
                ...
                for (uint32_t i=0; i < header_contigs->len; i++) 
                    if (ENT (Contig, *header_contigs, i)->chrom_index == r->chrom) {
                        txt_chrom = i;
                        break; // found
                    }
...
void ref_contigs_load_contigs (Reference ref)
{
    ...
    contigs_create_index (&ref->ctgs, SORT_BY_ALL);
...
void ref_contigs_get (Reference ref, ConstContigPkgP *contigs)
{
    *contigs = &ref->ctgs;
}
...
{
    return (uint32_t)ref->ctgs.contigs.len;
}

// binary search for this chrom in loaded_contigs_by_name. we count on gcc tail recursion optimization to keep this fast.
static WordIndex ref_contigs_get_by_name_do (Reference ref, const char *chrom_name, uint32_t chrom_name_len, 
                                             WordIndex first_sorted_index, WordIndex last_sorted_index)
{
    ...
    if (ref) {
        word_index = *ENT (WordIndex, ref->ctgs.by_name, mid_sorted_index);
        Contig *mid_word = ENT (Contig, ref->ctgs.contigs, word_index);
        snip = ENT (const char, ref->ctgs.dict, mid_word->char_index);
...
WordIndex ref_contigs_get_by_name (Reference ref, const char *chrom_name, uint32_t chrom_name_len, bool alt_ok, bool soft_fail)
{
    WordIndex ref_index = ref_contigs_get_by_name_do (ref, chrom_name, chrom_name_len, 0, 
                                                      ref ? ref->ctgs.by_name.len-1 : z_file->chroms_sorted_index.len-1);

    // case alt_ok: if not found, try common alternative names
    if (alt_ok && ref_index == WORD_INDEX_NONE) 
        ref_index = contigs_get_matching (&ref->ctgs, STRa(chrom_name), 0, NULL);
...
// ZIP: gets matching contig in the reference (same or alt). Returns true if a match was found.
bool ref_contigs_get_matching (Reference ref, PosType LN, STRp(txt_chrom), // in
                               STRp(*ref_contig), 
                               bool *is_alt, // out - true if ref_contig is an alternate chrom different than txt_chrom
                               int32_t *chrom_name_growth) // optional out
{
    WordIndex ref_index = contigs_get_matching (&ref->ctgs, STRa(txt_chrom), 0, is_alt);
    
    if (ref_index != WORD_INDEX_NONE) {
        *ref_contig = ref_contigs_get_name (ref, ref_index, ref_contig_len);
...
{
    ASSERT (ref_index >= 0 && ref_index < ref->ctgs.contigs.len, "expecting ref_index=%d < ref->ctgs.contigs.len=%"PRIu64, 
            ref_index, ref->ctgs.contigs.len);

    Contig *rc = ENT (Contig, ref->ctgs.contigs, ref_index);

    if (contig_name_len) *contig_name_len = rc->snip_len;
    
    return ENT (const char, ref->ctgs.dict, rc->char_index);
}

const char *ref_contigs_get_name_by_chrom_index (Reference ref, WordIndex chrom_index, const char **snip, uint32_t *snip_len)
{
    const Contig *contig = ref_contigs_get_contig_by_chrom_index (ref, chrom_index, false);
...
{               
    ...
    if (buf_is_alloc (&ref->ranges)) { // it is not allocated in --sex/coverage
        PosType ref_last_pos = ENT (Range, ref->ranges, ref_contig_index)->last_pos; // get from ranges because Contig.LN=0 - we don't populate it at reference creation
        ...
        // case: file header specifies length - it must be the same as the reference
            ...
            ASSINP (false, fmt, txt_name, chrom_name_len, chrom_name, *last_pos, ref->filename, ref_contig_name, ref_last_pos);
        }
    }
...
static const Contig *ref_contigs_get_contig_do (const Reference ref, WordIndex chrom_index, int32_t start_i, int32_t end_i)
{
...
const Contig *ref_contigs_get_contig_by_chrom_index (const Reference ref, WordIndex chrom_index, bool soft_fail)
{
    const Contig *rc = ref_contigs_get_contig_do (ref, chrom_index, 0, ref->ctgs.contigs.len-1);
...
void ref_contigs_iterate (const Reference ref, RefContigsIteratorCallback callback, void *callback_param)
{
    ...
    for (uint64_t i=0; i < rc_len; i++) {
        const char *ref_chrom_name = ENT (const char, ref->ctgs.dict, rc[i].char_index); 
...
PosType ref_contigs_get_genome_nbases (const Reference ref)
{
    ...
    ARRAY (const Contig, rc, ref->ctgs.contigs);
    const Contig *rc_with_largest_gpos = &rc[0];
    for (uint64_t i=1; i < rc_len; i++) 
                                   ...
                                   PosType *max_pos) // optional out
{
    ...
    const Contig *ctg;
    if (txt_chrom_index < ref->ctgs.contigs.len &&
           (!chrom_name_len ||
              ((ctg = ENT (Contig, ref->ctgs.contigs, txt_chrom_index))->snip_len == chrom_name_len &&
                !memcmp (ENT (char, ref->ctgs.dict, ctg->char_index), chrom_name, chrom_name_len))))  {
        
        // note: we compare the name too, because txt_chrom isn't always the same as ref_contig_index. example: VCF file in --chain, where VCF_CHROM is populated
    ...
    if (max_pos && contig_index != WORD_INDEX_NONE) 
        *max_pos = ENT (Contig, ref->ctgs.contigs, contig_index)->max_pos;
