...
static void main_genols (const char *z_filename, bool finalize, const char *subdir, bool recursive) 
{
    ...
    DataType dt = (z_file->data_type == DT_SAM && z_file->flags.genozip_header.txt_is_bin) ? DT_BAM : z_file->data_type;
...
static void main_genounzip (const char *z_filename, const char *txt_filename, bool is_last_file)
{
    ...
    if (z_file->data_type == DT_SAM && z_file->flags.genozip_header.txt_is_bin && flag.out_dt==-1 && exe_type == EXE_GENOCAT)
        flag.out_dt = DT_SAM;
...
static void main_genozip_open_z_file (char **z_filename)
{
    ...
    if (z_file->data_type == DT_BAM) z_file->flags.genozip_header.txt_is_bin = true; // compressed file is stored in binary form, and sizes are of the binary file
}
