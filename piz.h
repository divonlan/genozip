...
#define ASSPIZ(condition, format, ...)       do { if (!(condition)) { progress_newline; fprintf (stderr, "Error in %s:%u vb_i=%u line_i=%u: ", __FUNCTION__, __LINE__, vb->vblock_i, vb->line_i); fprintf (stderr, (format), __VA_ARGS__); fprintf (stderr, "\n"); exit_on_error(true); }} while(0)
#define ASSPIZ0(condition, string)           do { if (!(condition)) { progress_newline; fprintf (stderr, "Error in %s:%u vb_i=%u line_i=%u: %s\n", __FUNCTION__, __LINE__, vb->vblock_i, vb->line_i, string); exit_on_error(true); }} while(0)
