...
#define ASSPIZ(condition, format, ...) do { if (!(condition)) { progress_newline(); fprintf (stderr, "Error in %s:%u vb=%s/%u line_in_file(1-based)=%"PRIu64" vb->line_i(0-based)=%"PRId64"%s%s%s%s: ", __FUNCLINE, comp_name(vb->comp_i), vb->vblock_i, vb->line_i, vb->line_i ? (vb->line_i - vb->first_line) : 0, (Z_DT(DT_VCF) ? " sample_i=" : ""), (Z_DT(DT_VCF) ? str_int_s (vb->sample_i).s : ""), piz_dis_coords((VBlockP)(vb)).s, piz_dis_qname((VBlockP)(vb)).s); fprintf (stderr, (format), __VA_ARGS__); fprintf (stderr, "\n"); exit_on_error(true); }} while(0)
